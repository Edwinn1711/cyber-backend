    ...
    <div className="flex h-screen w-full text-slate-100 overflow-hidden font-sans selection:bg-fuchsia-500/30">
      ...
      <motion.aside initial={false} animate={{ width: isSidebarCollapsed ? 80 : 250 }} className="h-screen bg-[#05050A]/95 backdrop-blur-3xl border-r border-white/10 flex flex-col z-[100] transition-all duration-500 shadow-2xl">
        ...
        <nav className="flex-1 px-4 py-8 space-y-3">
          ...
          <button onClick={() => setAppFeedbackModal(true)} className="w-full flex items-center p-3.5 rounded-xl text-slate-400 hover:text-fuchsia-400 hover:bg-white/5 transition-all gap-4 group">
            <Lightbulb size={18} className="group-hover:animate-pulse" /> {!isSidebarCollapsed && <span className="font-bold text-[9px] tracking-widest uppercase">FEEDBACK</span>}
          </button>
        </nav>
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
        <header className="h-20 flex items-center justify-between px-10 border-b border-white/10 bg-black/40 backdrop-blur-xl">
            <div className="flex items-center gap-3 px-5 py-2 bg-white/5 border border-white/20 rounded-full shadow-inner"><div className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse shadow-[0_0_10px_#34d399]" /><span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">GATEWAY ACTIVE</span></div>
            <div className="flex items-center gap-6 text-right"><div className="hidden sm:block"><p className="text-[11px] font-black text-white tracking-widest uppercase">{user.username}</p><p className="text-[9px] font-bold text-fuchsia-400 uppercase tracking-widest mt-1">OPERATIVE MODE</p></div><div className="w-10 h-10 bg-gradient-to-br from-indigo-500 to-fuchsia-600 rounded-full flex items-center justify-center border border-white/20 shadow-lg"><User size={18} /></div></div>
        </header>
          ...
          <AnimatePresence mode="wait">
            ...
            {view === 'dashboard' && (
              <motion.div key="dash" variants={containerVariants} initial="hidden" animate="show" exit={{opacity:0, y:-10}} className="max-w-[1300px] mx-auto space-y-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                   <motion.div variants={itemVariants} whileHover={{y:-5}} transition={{type:'spring',stiffness:400,damping:20}} className="bg-[#05050A]/90 border border-white/5 p-10 rounded-[3rem] flex flex-col items-center justify-center text-center hover:border-fuchsia-500/40 transition-all shadow-2xl relative overflow-hidden group">
                   ...
                   <motion.div variants={itemVariants} whileHover={{y:-5}} transition={{type:'spring',stiffness:400,damping:20}} className="bg-[#05050A]/90 border border-white/5 p-10 rounded-[3rem] flex flex-col items-center justify-center hover:border-fuchsia-500/40 transition-all shadow-2xl group text-center">
                      <div className="relative mb-6 flex justify-center w-full transform group-hover:scale-105 transition-all">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-6xl font-black text-white tracking-tighter">{score}</span><span className="text-[8px] font-black text-slate-500 uppercase mt-1">AVG SCORE</span></div>
                      </div>
                      <div className={`px-8 py-2 rounded-full border border-white/10 text-[10px] font-black tracking-widest uppercase bg-black/40 ${getScoreData(score).text} shadow-lg shadow-black/50`}>{getScoreData(score).label}</div>
                   </motion.div>
                   <motion.div variants={itemVariants} whileHover={{y:-5}} transition={{type:'spring',stiffness:400,damping:20}} className="bg-[#05050A]/90 border border-white/5 p-10 rounded-[3rem] hover:border-fuchsia-500/40 transition-all shadow-2xl flex flex-col justify-between group text-center"><p className="text-[10px] font-black text-slate-500 tracking-[0.5em] mb-10 uppercase flex items-center justify-center gap-3"><Activity size={16} className="text-fuchsia-500 animate-pulse"/> SYSTEM ANALYTICS</p>
                  ...
                  {TACTICAL_DOMAINS.map((domain, i) => {
                    ...
                    return (
                      <motion.div 
                        ...
                        className={`bg-[#05050A]/95 border ${done ? 'border-fuchsia-500/50 shadow-[0_0_30px_rgba(217,70,239,0.15)]' : 'border-white/5'} p-10 lg:p-12 rounded-[3.5rem] lg:rounded-[4rem] cursor-pointer group transition-all duration-300 shadow-2xl relative overflow-hidden`}
                      >
                         {/* Deep Glow Effect on Hover */}
                         ...
                         <div className="flex justify-between items-start mb-12 relative z-10">
                            <div className="w-16 lg:w-18 h-16 lg:h-18 rounded-[1.5rem] lg:rounded-[2rem] flex items-center justify-center shadow-[0_0_15px_rgba(0,0,0,0.5)] group-hover:shadow-xl transition-all" style={{ backgroundColor: `${domain.color}15`, border: `1px solid ${domain.color}30` }}>
                               <domain.icon size={30} style={{ color: domain.color }} />
                            </div>
                            {done && <div className="bg-fuchsia-600 px-4 py-1.5 rounded-full border border-white/10 shadow-lg"><span className="text-[10px] font-black text-white tracking-widest uppercase">Verified</span></div>}
                         </div>
                         
                         <h3 className="text-2xl lg:text-3xl font-black text-white mb-4 uppercase tracking-tighter leading-none relative z-10 group-hover:text-fuchsia-100 transition-colors">{domain.title}</h3>
                         <p className="text-[11px] lg:text-[12px] text-slate-400 font-bold tracking-widest uppercase mb-12 leading-relaxed opacity-80 relative z-10">{domain.desc}</p>
                         
                         <div className="mt-auto pt-8 border-t border-white/5 flex justify-end items-center relative z-10">
                            <span className="text-[11px] font-black tracking-[0.5em] text-slate-500 group-hover:text-fuchsia-400 group-hover:translate-x-3 transition-all uppercase">
                               INITIALIZE PROTOCOL <ArrowRight size={16} className="inline ml-4" />
                            </span>
                         </div>
                      </motion.div>
                    );
            ...
            {view === 'briefing' && (
              <motion.div key="briefing" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-4xl mx-auto flex flex-col items-center justify-center min-h-[75vh]">
                 ...
                 <motion.div className="flex flex-col items-center text-center space-y-10"><div className="px-8 py-3 rounded-full bg-emerald-500/10 border-2 border-emerald-400 text-emerald-400 text-[12px] font-black tracking-[0.7em] uppercase shadow-[0_0_30px_rgba(52,211,153,0.2)] animate-pulse">Uplink Confirmed</div><h2 className="text-6xl lg:text-7xl font-black text-white tracking-tighter uppercase leading-none drop-shadow-2xl text-center">Transmission Received.</h2><p className="text-[16px] font-black text-white leading-relaxed tracking-[0.3em] uppercase max-w-2xl opacity-100 mx-auto text-center">Halo <span className="text-fuchsia-400">{user.username}</span>, inisiasi kuis untuk topik <span className="text-fuchsia-400">{selectedDomain}</span> sekarang?</p></motion.div>
