    ...
    <div className="flex h-screen w-full text-slate-200 overflow-hidden font-sans selection:bg-fuchsia-500/30">
      ...
      <motion.aside initial={false} animate={{ width: isSidebarCollapsed ? 80 : 280 }} className="h-screen bg-[#050505]/90 backdrop-blur-3xl border-r border-white/5 flex flex-col z-[100] transition-all duration-500 relative">
        ...
        <nav className="flex-1 px-4 py-8 space-y-2">
          <button onClick={() => handleSafeViewChange('dashboard')} className={`w-full flex items-center p-4 rounded-xl transition-all gap-4 ${view === 'dashboard' ? 'bg-fuchsia-500/10 text-fuchsia-400 border border-fuchsia-500/20 shadow-lg' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}>
            ...
            {view === 'dashboard' && (
              <motion.div key="dash" {...portalTransition} className="max-w-6xl mx-auto space-y-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-4">
                   {/* Profile Card (Centered Content) */}
                   <div className="bg-[#09090b]/80 backdrop-blur-xl border border-white/5 p-10 rounded-[2.5rem] flex flex-col items-center justify-center text-center hover:border-fuchsia-500/30 transition-all shadow-2xl relative overflow-hidden group">
                        <div className="w-20 h-20 bg-gradient-to-b from-fuchsia-600 to-purple-900 rounded-3xl flex items-center justify-center text-white border border-fuchsia-500/50 mb-8 shadow-xl"><Fingerprint size={40} /></div>
                        <h4 className="text-3xl font-black text-white tracking-widest uppercase">{user.username}</h4>
                        <p className="text-[11px] text-fuchsia-400 font-bold tracking-[0.3em] uppercase mt-4">{user.class_name || "BELUM TERDAFTAR"}</p>
                   </div>

                   {/* Average Score Card (Centered) */}
                   <div className="bg-[#09090b]/80 backdrop-blur-xl border border-white/5 p-10 rounded-[2.5rem] flex flex-col items-center justify-center hover:border-fuchsia-500/30 transition-all shadow-2xl group">
                      <div className="relative mb-6 flex justify-center w-full transform group-hover:scale-105 transition-all duration-500">
                        <svg className="w-40 h-40 transform -rotate-90">
                          <circle cx="80" cy="80" r="72" stroke="rgba(255,255,255,0.03)" strokeWidth="6" fill="transparent" />
                          <motion.circle initial={{ strokeDasharray: "0 1000" }} animate={{ strokeDasharray: `${(score / 100) * 452} 1000` }} transition={{ duration: 2.5, ease: "easeOut" }} cx="80" cy="80" r="72" stroke="#d946ef" strokeWidth="6" strokeLinecap="round" fill="transparent" style={{ filter: 'drop-shadow(0 0 15px rgba(217,70,239,0.7))' }} />
                        </svg>
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <span className="text-6xl font-black text-white tracking-tighter">{score}</span>
                          <span className="text-[9px] font-bold text-slate-500 uppercase mt-1 tracking-widest">Rata-rata</span>
                        </div>
                      </div>
                      <div className="px-8 py-2 rounded-full bg-fuchsia-500/10 border border-fuchsia-500/30 text-fuchsia-400 font-black text-[10px] tracking-[0.4em] uppercase shadow-lg">{readiness.label}</div>
                   </div>

                   {/* System Status Card (Centered) */}
                   <div className="bg-[#09090b]/80 backdrop-blur-xl border border-white/5 p-10 rounded-[2.5rem] hover:border-fuchsia-500/30 transition-all shadow-2xl flex flex-col items-center justify-center text-center group">
                      <p className="text-[10px] font-black text-slate-600 tracking-[0.4em] mb-10 uppercase flex items-center gap-3"><Activity size={14} className="text-fuchsia-500"/> SISTEM STATUS</p>
                      <div className="space-y-6 font-mono text-[10px] tracking-[0.2em] text-slate-400 w-full px-4">
                        <div className="flex justify-between border-b border-white/5 pb-3"><span>SERVER</span><span className="text-fuchsia-400 font-bold uppercase">Online</span></div>
                        <div className="flex justify-between border-b border-white/5 pb-3"><span>ENKRIPSI</span><span className="text-emerald-500 font-bold uppercase">AES-256</span></div>
                        <div className="flex justify-between"><span>DATABASE</span><span className="text-white font-bold opacity-80 uppercase">SINKRON</span></div>
                      </div>
                   </div>

                   {/* --- BROADCAST CENTER (MENGISI KEKOSONGAN DI BAWAH) --- */}
                   <div className="lg:col-span-3 bg-gradient-to-br from-[#09090b] to-[#050505] border border-white/5 p-10 rounded-[3rem] relative overflow-hidden flex flex-col lg:flex-row items-center justify-between gap-10 shadow-2xl group hover:border-fuchsia-500/30 transition-all mt-6">
                      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-fuchsia-600/5 blur-[120px] rounded-full pointer-events-none -z-10" />
                      <div className="flex items-center gap-10 z-10 text-center lg:text-left">
                        <div className="w-20 h-20 bg-[#070707] rounded-3xl flex items-center justify-center shrink-0 border border-white/5 shadow-2xl"><BellRing size={32} className="text-fuchsia-500 animate-bounce" /></div>
                      ...
                      <button onClick={handleStartMissionClick} className="px-14 py-6 bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white rounded-2xl font-black text-[12px] tracking-[0.5em] hover:scale-[1.05] active:scale-95 transition-all shadow-[0_15px_40px_rgba(217,70,239,0.4)] flex items-center gap-5 uppercase z-10">PROSES READINESS TEST <Zap size={20} /></button>
                   </div>
                </div>
              </motion.div>
            )}
      ...
      <AnimatePresence>
        {detailModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[3000] flex items-center justify-center p-6 bg-black/95 backdrop-blur-md">
            <motion.div initial={{ scale: 0.95, y: 15 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-4xl max-h-[85vh] bg-[#09090b] border border-white/10 rounded-[3rem] p-12 shadow-2xl flex flex-col overflow-hidden text-center text-center">
