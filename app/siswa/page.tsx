  ...
  return (
    <div className="fixed inset-0 z-0 overflow-hidden bg-black">
      <AnimatePresence mode="wait">
        <motion.img key={bgIdx} src={CYBER_ASSETS[bgIdx]} initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 0.3, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 4 }} className="absolute inset-0 w-full h-full object-cover pointer-events-none" />
      </AnimatePresence>
...
const RobotFace = ({ size = 24, ringSize = "w-40 h-40", coreSize = "w-24 h-24" }) => {
  ...
  useEffect(() => {
    const handleMove = (e: MouseEvent | TouchEvent) => {
      ...
      const clientX = 'touches' in e ? e.touches[0].clientX : (e as MouseEvent).clientX;
      const clientY = 'touches' in e ? e.touches[0].clientY : (e as MouseEvent).clientY;
      const moveX = ((clientX - centerX) / window.innerWidth) * 36;
      const moveY = ((clientY - centerY) / window.innerHeight) * 36;
      setOffset({ x: Math.max(-12, Math.min(12, moveX)), y: Math.max(-12, Math.min(12, moveY)) });
    };
  ...
  return (
    <div className="relative flex items-center justify-center" ref={faceRef}>
      ...
      <div className={`relative ${coreSize} bg-[#050505] border-2 border-fuchsia-500 rounded-full shadow-[0_0_40px_rgba(217,70,239,0.5)] flex flex-col items-center justify-center z-10 overflow-hidden`}>
         <motion.div animate={{ x: offset.x, y: offset.y }} transition={{ type: "spring", stiffness: 150, damping: 15 }} className="flex flex-col items-center justify-center z-20">
...
export default function StudentGodTierDashboard() {
  ...
  return (
    <div className="flex h-screen w-full text-slate-200 overflow-hidden font-sans selection:bg-fuchsia-500/30">
      ...
      {/* --- SIDEBAR --- */}
      <motion.aside initial={false} animate={{ width: isSidebarCollapsed ? 80 : 280 }} className="h-screen bg-[#050505]/90 backdrop-blur-2xl border-r border-white/5 flex flex-col z-[100] transition-all duration-500">
        <div className="h-20 px-6 flex items-center justify-between border-b border-white/5">
          {!isSidebarCollapsed && <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-gradient-to-br from-fuchsia-600 to-purple-800 flex items-center justify-center shadow-lg"><ShieldCheck size={18} className="text-white" /></div><span className="font-bold tracking-widest text-sm text-white uppercase tracking-[0.2em]">CYBER EDU</span></div>}
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
        <header className="h-20 flex items-center justify-between px-12 border-b border-white/5 bg-[#050505]/60 backdrop-blur-md">
            ...
            <div className="flex items-center gap-6"><div className="text-right"><p className="text-[11px] font-black text-white uppercase">{user.username}</p><p className="text-[9px] font-bold text-slate-500 uppercase">{user.class_name || "UNASSIGNED"}</p></div><div className="w-10 h-10 bg-gradient-to-br from-fuchsia-600 to-purple-800 rounded-full flex items-center justify-center text-white border border-white/10 shadow-lg"><User size={18} /></div></div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto no-scrollbar px-12 py-10" ref={scrollRef}>
          <AnimatePresence mode="wait">
            ...
            {/* VIEW DASHBOARD */}
            {view === 'dashboard' && (
              <motion.div key="dash" {...portalTransition} className="max-w-6xl mx-auto space-y-8 text-center lg:text-left">
                <div className="font-mono text-[9px] text-slate-500 tracking-[0.3em] uppercase mb-8 opacity-60">STATUS AKADEMIK: AKTIF • PROTOKOL UJIAN V2.1</div>
                <div className="space-y-2"><h1 className="text-5xl font-black text-white tracking-tighter leading-none uppercase">PORTAL <span className="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-purple-600">SISWA.</span></h1><p className="text-slate-500 text-[11px] font-bold tracking-[0.2em] uppercase">Sistem Evaluasi Siber Terpadu</p></div>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-6">
                   <div className="bg-[#09090b]/80 border border-white/5 p-8 rounded-[2rem] flex flex-col items-center justify-center text-center hover:border-fuchsia-500/30 transition-all shadow-2xl">
                        <div className="w-16 h-16 bg-gradient-to-b from-fuchsia-600 to-purple-900 rounded-2xl flex items-center justify-center text-white border border-fuchsia-500/50 mb-6 shadow-xl"><Fingerprint size={32} /></div>
                        <h4 className="text-2xl font-black text-white tracking-widest uppercase">{user.username}</h4>
                        <p className="text-[10px] text-fuchsia-400 font-bold tracking-widest uppercase mt-2">{user.class_name}</p>
                   </div>
                   <div className="bg-[#09090b]/80 border border-white/5 p-8 rounded-[2rem] flex flex-col items-center justify-center hover:border-fuchsia-500/30 transition-all shadow-2xl">
                      <div className="relative mb-4 flex justify-center w-full">
                        <svg className="w-32 h-32 transform -rotate-90">
                          <circle cx="64" cy="64" r="58" stroke="rgba(255,255,255,0.03)" strokeWidth="5" fill="transparent" />
                          <motion.circle initial={{ strokeDasharray: "0 1000" }} animate={{ strokeDasharray: `${(score / 100) * 365} 1000` }} transition={{ duration: 2 }} cx="64" cy="64" r="58" stroke="#d946ef" strokeWidth="5" strokeLinecap="round" fill="transparent" />
                        </svg>
                        <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-4xl font-black text-white">{score}</span><span className="text-[7px] font-bold text-slate-500 uppercase mt-1">Rata-rata</span></div>
                      </div>
                      <div className="px-5 py-1.5 rounded-full bg-fuchsia-500/10 border border-fuchsia-500/30 text-fuchsia-400 font-black text-[8px] tracking-[0.3em] uppercase">{readiness.label}</div>
                   </div>
                   <div className="bg-[#09090b]/80 border border-white/5 p-8 rounded-[2rem] hover:border-fuchsia-500/30 transition-all shadow-2xl flex flex-col items-center justify-center text-center"><p className="text-[9px] font-black text-slate-600 tracking-[0.3em] mb-6 uppercase">Sistem Status</p>
                      <div className="space-y-4 font-mono text-[9px] tracking-[0.1em] text-slate-400 w-full">
                        <div className="flex justify-between border-b border-white/5 pb-2"><span>SERVER</span><span className="text-fuchsia-400">ONLINE</span></div>
                        <div className="flex justify-between border-b border-white/5 pb-2"><span>ENKRIPSI</span><span className="text-emerald-500">AES-256</span></div>
                        <div className="flex justify-between"><span>DATABASE</span><span className="text-slate-500">SINKRON</span></div>
                      </div>
                   </div>
                </div>
              </motion.div>
            ...
            {/* VIEW REPORTS - KESIMPULAN (CENTER ALIGNED) */}
            {view === 'reports' && (
              <motion.div key="reports" {...portalTransition} className="max-w-6xl mx-auto space-y-12">
                 <div className="space-y-3 text-center">
                    <h2 className="text-5xl font-black text-white tracking-widest uppercase">Kesimpulan Evaluasi.</h2>
                 ...
                 <div className="bg-[#09090b]/90 backdrop-blur-2xl border border-white/5 rounded-[3rem] p-12 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12 relative z-10">
                        <div className="bg-[#050505]/60 rounded-3xl p-8 border border-white/5 flex flex-col items-center text-center group hover:border-fuchsia-500/20 transition-all">
                           ...
                           <p className="text-5xl text-white font-black font-mono tracking-tighter">{history.length}</p>
                        </div>
                        <div className="bg-[#050505]/60 rounded-3xl p-8 border border-white/5 flex flex-col items-center text-center group hover:border-fuchsia-500/20 transition-all">
                           <p className="text-[10px] text-slate-500 font-black tracking-[0.4em] mb-4 uppercase">NILAI RATA-RATA</p>
                           <p className="text-5xl text-fuchsia-400 font-black font-mono tracking-tighter">{score}%</p>
                        </div>
                        <div className="bg-[#050505]/60 rounded-3xl p-8 border border-white/5 flex flex-col items-center text-center group hover:border-fuchsia-500/20 transition-all">
                           <p className="text-[10px] text-slate-500 font-black tracking-[0.4em] mb-4 uppercase">PREDIKAT</p>
                           <p className={`text-2xl font-black tracking-[0.2em] uppercase mt-2 ${readiness.color === '#ef4444' ? 'text-red-500' : readiness.color === '#eab308' ? 'text-yellow-500' : 'text-fuchsia-400'}`}>
                              {readiness.label}
                           </p>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-center border-collapse">
                          <thead>
                            <tr className="border-b border-white/5 bg-white/[0.03]">
                              <th className="py-6 text-[9px] font-black tracking-[0.4em] text-slate-500 uppercase">Sesi</th>
                              <th className="py-6 text-[9px] font-black tracking-[0.4em] text-slate-500 uppercase">Sektor Target</th>
                              <th className="py-6 text-[9px] font-black tracking-[0.4em] text-slate-500 uppercase">Akurasi</th>
                              <th className="py-6 text-[9px] font-black tracking-[0.4em] text-slate-500 uppercase">Keterangan</th>
                              <th className="py-6 text-[9px] font-black tracking-[0.4em] text-slate-500 uppercase">Detail</th>
                            </tr>
                          </thead>
                          <tbody>
                             {history.length === 0 ? (
                                <tr><td colSpan={5} className="py-20 text-slate-600 font-black text-xs uppercase tracking-widest">RIWAYAT KOSONG</td></tr>
                             ) : (
                                history.map((h, i) => {
                  ...
                  {TACTICAL_DOMAINS.map((domain, i) => {
                    ...
                    return (
                      <motion.div key={i} whileHover={{ y: -8 }} onClick={() => { if(done) { setPendingDomain(domain.id); setShowRetakeWarning(true); } else { setSelectedDomain(domain.id); setView('briefing'); } }} className={`bg-[#09090b]/90 border ${done ? 'border-fuchsia-500/40 shadow-lg' : 'border-white/5'} p-8 rounded-[2rem] cursor-pointer group flex flex-col transition-all hover:border-fuchsia-500/30 overflow-hidden`}>
                        <div className="flex justify-between items-start mb-8"><div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-xl" style={{ backgroundColor: `${domain.color}10`, border: `1px solid ${domain.color}30` }}><domain.icon size={24} style={{ color: domain.color }} /></div>{done && <div className="bg-fuchsia-500/10 px-3 py-1 rounded-full border border-fuchsia-500/20"><span className="text-[8px] font-black text-fuchsia-400 tracking-widest uppercase">Tuntas</span></div>}</div>
                        <h3 className="text-lg font-black text-white mb-2 uppercase tracking-widest">{domain.title}</h3><p className="text-[10px] text-slate-500 font-bold tracking-[0.2em] uppercase mb-8 line-clamp-2">{domain.desc}</p>
                        <div className="mt-auto pt-5 border-t border-white/5 flex justify-end items-center"><span className="text-[9px] font-black tracking-widest text-slate-400 group-hover:text-fuchsia-400 transition-all uppercase">Pilih Sektor <ArrowRight size={12} className="inline ml-2" /></span></div>
                      </motion.div>
                    );
                  })}
                </div>
            ...
            {view === 'mission' && (
              <motion.div key="mission" {...portalTransition} className="max-w-4xl mx-auto space-y-10 pb-40">
                ...
                <div className="space-y-6">
                   ...
                   )) : <div className="text-center py-20 border border-dashed border-white/5 rounded-3xl uppercase text-slate-600">DATA SOAL TIDAK DITEMUKAN.</div>}
                </div>
