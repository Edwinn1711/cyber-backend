  ...
  useEffect(() => {
    const handleInteraction = (e: MouseEvent) => {
      const newParticles = Array.from({ length: 8 }).map((_, i) => ({
        id: Math.random(), x: e.clientX, y: e.clientY, angle: (Math.PI * 2 / 8) * i, velocity: Math.random() * 50 + 20
      }));
    ...
    <div className="flex h-screen w-full text-slate-100 overflow-hidden font-sans selection:bg-fuchsia-500/30">
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
        <header className="h-20 flex items-center justify-between px-10 border-b border-white/10 bg-black/40 backdrop-blur-xl">
            ...
            <div className="flex items-center gap-6 text-right"><div className="hidden sm:block"><p className="text-[11px] font-black text-white tracking-widest uppercase">{user.username}</p><p className="text-[10px] font-bold text-fuchsia-400 uppercase tracking-[0.2em] mt-1">OPERATIVE MODE</p></div><div className="w-11 h-11 bg-gradient-to-br from-indigo-500 to-fuchsia-600 rounded-full flex items-center justify-center border border-white/20 shadow-lg"><User size={20} /></div></div>
        </header>
          ...
          <AnimatePresence mode="wait">
            ...
            {view === 'dashboard' && (
              <motion.div key="dash" variants={containerVariants} initial="hidden" animate="show" className="max-w-[1300px] mx-auto space-y-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-10 rounded-[3rem] flex flex-col items-center justify-center text-center hover:border-fuchsia-500/40 transition-all shadow-2xl relative overflow-hidden group"><div className="absolute top-0 left-0 w-full h-1 bg-fuchsia-500 shadow-[0_0_15px_#d946ef] animate-scan opacity-0 group-hover:opacity-100" /><div className="w-20 h-20 bg-gradient-to-b from-fuchsia-600 to-indigo-900 rounded-3xl flex items-center justify-center text-white border border-white/10 mb-8 transform group-hover:scale-110 transition-all duration-500"><Fingerprint size={36} /></div><h4 className="text-2xl font-black text-white tracking-widest uppercase leading-none">{user.username}</h4><p className="text-[10px] text-fuchsia-400 font-black tracking-[0.4em] uppercase mt-4">{user.class_name || "NOT ASSIGNED"}</p></motion.div>
                   ...
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-10 rounded-[3rem] hover:border-fuchsia-500/40 transition-all shadow-2xl flex flex-col justify-between group text-center"><p className="text-[10px] font-black text-slate-500 tracking-[0.5em] mb-10 uppercase flex items-center justify-center gap-3"><Activity size={16} className="text-fuchsia-500 animate-pulse"/> SYSTEM ANALYTICS</p><div className="space-y-6 font-mono text-[10px] tracking-[0.2em] text-slate-300 w-full px-4"><div className="flex justify-between border-b border-white/5 pb-3"><span>HOST</span><span className="text-fuchsia-400 font-bold uppercase">VERCEL</span></div><div className="flex justify-between border-b border-white/5 pb-3"><span>SECURITY</span><span className="text-emerald-500 font-bold uppercase">AES-256</span></div><div className="flex justify-between"><span>DATABASE</span><span className="text-white font-bold uppercase opacity-80">SINKRON</span></div></div></motion.div>
            ...
            {view === 'briefing' && (
              <motion.div key="briefing" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-4xl mx-auto flex flex-col items-center justify-center min-h-[75vh]">
                 ...
                 <div className="flex gap-10 mt-24"><button onClick={() => setView('assessment')} className="px-16 py-6 border-4 border-white/10 text-slate-300 rounded-[2.5rem] font-black text-[12px] tracking-[0.5em] uppercase hover:border-white/40 hover:text-white transition-all">Cancel</button><button onClick={handleStartMissionClick} className="px-16 py-6 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white rounded-[2.5rem] font-black text-[12px] tracking-[0.5em] shadow-[0_20px_60px_rgba(217,70,239,0.5)] hover:scale-105 transition-all uppercase flex items-center gap-6">Execute Protocol <Zap size={24} /></button></div>
              </motion.div>
            )}
