...
const ParticleBurstClickEffect = () => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] pointer-events-none overflow-hidden">
      <AnimatePresence>
        {particles.map((p) => (
          <motion.div key={p.id} initial={{ scale: 0, opacity: 1, x: p.x, y: p.y }} animate={{ scale: [0, 1.5, 0], opacity: [1, 0.8, 0], x: p.x + Math.cos(p.angle) * p.velocity, y: p.y + Math.sin(p.angle) * p.velocity }} transition={{ duration: 0.8 }} className="absolute rounded-full bg-fuchsia-400 shadow-[0_0_20px_#d946ef]" style={{ width: '4px', height: '4px' }} />
        ))}
      </AnimatePresence>
    </div>
  );
    ...
    <div className="flex h-screen w-full text-slate-100 overflow-hidden font-sans selection:bg-fuchsia-500/30">
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
        <header className="h-20 flex items-center justify-between px-6 lg:px-10 border-b border-white/10 bg-black/40 backdrop-blur-xl">
            <div className="flex items-center gap-3 px-4 py-2 bg-white/5 border border-white/20 rounded-full shadow-inner"><div className="w-2.5 h-2.5 bg-emerald-400 rounded-full animate-pulse shadow-[0_0_15px_#34d399]" /><span className="text-[8px] lg:text-[9px] font-black text-slate-200 uppercase tracking-widest">GATEWAY ACTIVE</span></div>
            <div className="flex items-center gap-4 lg:gap-6 text-right"><div className="hidden sm:block"><p className="text-[11px] font-black text-white tracking-widest uppercase">{user.username}</p><p className="text-[9px] font-bold text-fuchsia-400 uppercase tracking-[0.2em] mt-1">OPERATIVE MODE</p></div><div className="w-11 h-11 bg-gradient-to-br from-indigo-500 to-fuchsia-600 rounded-full flex items-center justify-center border border-white/20 shadow-lg"><User size={20} /></div></div>
        </header>

        <main className="flex-1 overflow-y-auto no-scrollbar px-6 lg:px-14 py-10" ref={scrollRef}>
          <AnimatePresence mode="wait">
            
            {/* VIEW DASHBOARD (FIXED RESPONSIVE ALIGNMENT) */}
            {view === 'dashboard' && (
              <motion.div key="dash" variants={containerVariants} initial="hidden" animate="show" className="max-w-[1300px] mx-auto space-y-12">
                <motion.div variants={itemVariants} className="text-center space-y-4 pt-4">
                  <div className="text-fuchsia-400 font-black text-[10px] tracking-[0.6em] uppercase mb-2 flex items-center justify-center gap-3">{getGreeting()} OPERATIVE</div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                   ...
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-8 lg:p-10 rounded-[2.5rem] lg:rounded-[3rem] flex flex-col items-center justify-center hover:border-fuchsia-500/40 transition-all shadow-2xl group text-center">
                      ...
                      <div className={`px-8 py-1.5 rounded-full border border-white/10 text-[9px] font-black tracking-widest uppercase bg-black/40 ${getScoreData(score).text} shadow-lg shadow-black/50`}>{getScoreData(score).label}</div>
                   </motion.div>
                   ...
                   {/* --- BROADCAST CENTER (FIXED MOBILE ALIGNMENT) --- */}
                   <motion.div variants={itemVariants} className="lg:col-span-3 bg-gradient-to-br from-[#0a0a14] to-[#020202] border border-white/10 p-8 lg:p-12 rounded-[2.5rem] lg:rounded-[4rem] relative overflow-hidden flex flex-col lg:flex-row items-center justify-between gap-10 lg:gap-12 shadow-[0_20px_60px_rgba(0,0,0,0.8)] group hover:border-fuchsia-500/40 transition-all duration-700">
                      ...
                      {/* Text & Icon Block - Centered on Mobile */}
                      <div className="flex flex-col lg:flex-row items-center gap-6 lg:gap-10 z-10 text-center lg:text-left">
                        <div className="w-16 lg:w-20 h-16 lg:h-20 bg-black/80 rounded-3xl flex items-center justify-center shrink-0 border border-white/10 shadow-2xl group-hover:scale-110 group-hover:rotate-6 transition-all duration-500">
                          <BellRing size={32} className="text-fuchsia-500 fill-current animate-bounce" />
                        </div>
                        <div className="space-y-3">
                          <div className="flex items-center justify-center lg:justify-start gap-4">
                             <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_15px_#10b981] animate-ping" />
                             <span className="text-[10px] lg:text-[11px] font-black tracking-[0.6em] text-emerald-400 uppercase">Mission Status: Active</span>
                          </div>
            ...
            {view === 'assessment' && (
              <motion.div key="assess-hub" variants={containerVariants} initial="hidden" animate="show" className="max-w-6xl mx-auto space-y-16">
                <motion.div variants={itemVariants} className="text-center space-y-4 pt-6"><h2 className="text-4xl lg:text-6xl font-black text-white tracking-widest uppercase leading-none drop-shadow-2xl">Target Modules.</h2><p className="text-fuchsia-400 text-[11px] font-black tracking-[0.6em] uppercase">Select strategic sector for diagnostic protocol</p></motion.div>
            ...
            {view === 'mission' && (
              <motion.div key="mission" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto space-y-12 pb-44 text-center lg:text-left">
                <header className="flex justify-between items-end border-b-2 border-white/10 pb-10 pt-6"><div className="space-y-4"><button onClick={() => setView('assessment')} className="text-[10px] font-black text-slate-400 hover:text-red-400 flex items-center gap-4 tracking-widest uppercase transition-all"><ArrowLeft size={20} /> ABORT SESSION</button><h2 className="text-4xl lg:text-6xl font-black text-white tracking-tighter uppercase leading-none">PHASE 0{currentStep}</h2><p className="text-fuchsia-500 font-black tracking-[0.6em] text-[11px] uppercase">PROTOCOL: {selectedDomain}</p></div><div className="text-[100px] lg:text-[140px] font-black text-white/5 font-mono leading-none tracking-tighter">0{currentStep}</div></header>
