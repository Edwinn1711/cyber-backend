...
// --- 5. TACTICAL DOMAINS CONFIG (KEMBALIKAN YANG HILANG) ---
const TACTICAL_DOMAINS = [
  ...
  { id: "Threat", title: "Threat Hunting", icon: RadarIcon, color: "#FB923C", desc: "Proactive Search" },
];
    ...
    <div className="flex h-screen w-full text-slate-100 overflow-hidden font-sans selection:bg-fuchsia-500/30">
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
        <header className="h-20 flex items-center justify-between px-10 border-b border-white/10 bg-black/40 backdrop-blur-xl">
            ...
            <div className="flex items-center gap-6 text-right"><div className="hidden sm:block"><p className="text-[12px] font-black text-white tracking-widest uppercase">{user.username}</p><p className="text-[10px] font-bold text-fuchsia-400 uppercase tracking-[0.2em] mt-1">OPERATIVE MODE</p></div><div className="w-11 h-11 bg-gradient-to-br from-violet-600 to-fuchsia-600 rounded-full flex items-center justify-center border border-white/20 shadow-lg"><User size={20} /></div></div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto no-scrollbar px-10 lg:px-14 py-10" ref={scrollRef}>
          <AnimatePresence mode="wait">
              ...
              <motion.div key="dash" variants={containerVariants} initial="hidden" animate="show" className="max-w-[1300px] mx-auto space-y-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-10 rounded-[3rem] flex flex-col items-center justify-center text-center hover:border-fuchsia-500/40 transition-all shadow-2xl relative overflow-hidden group">
                        <div className="absolute top-0 left-0 w-full h-1 bg-fuchsia-500 shadow-[0_0_15px_#d946ef] animate-scan" />
                   ...
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-10 rounded-[3rem] hover:border-fuchsia-500/40 transition-all shadow-2xl flex flex-col justify-between group text-center"><p className="text-[10px] font-black text-slate-500 tracking-[0.5em] mb-10 uppercase flex items-center justify-center gap-3"><Activity size={16} className="text-fuchsia-500 animate-pulse"/> SYSTEM ANALYTICS</p>
                      <div className="space-y-6 font-mono text-[10px] tracking-[0.2em] text-slate-300 w-full px-4">
                        <div className="flex justify-between border-b border-white/5 pb-3"><span>SERVER</span><span className="text-fuchsia-400 font-bold">READY</span></div>
                        <div className="flex justify-between border-b border-white/5 pb-3"><span>SECURITY</span><span className="text-emerald-500 font-bold">AES-256</span></div>
                        <div className="flex justify-between"><span>DATABASE</span><span className="text-white font-bold uppercase">SINKRON</span></div>
                      </div>
                   </motion.div>
            ...
            {/* VIEW REPORTS - RESULTS (TACTICAL COMMAND VERSION) */}
            {view === 'reports' && (
              <motion.div key="reports" variants={containerVariants} initial="hidden" animate="show" className="max-w-[1400px] mx-auto space-y-10">
                 ...
                 <motion.div variants={itemVariants} className="text-center space-y-4 pt-4">
                    ...
                    <p className="text-slate-500 text-[11px] font-bold tracking-[0.4em] uppercase max-w-xl mx-auto opacity-100">Historical operative data records summary from secure cloud vaults.</p>
                 </motion.div>

                 <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    {/* WIDGET 1: RADAR CHART (DEWA FEATURE) */}
                    <motion.div variants={itemVariants} className="lg:col-span-7 bg-[#0a0a0f]/90 border border-white/10 rounded-[3rem] p-10 flex flex-col md:flex-row items-center justify-between shadow-2xl relative overflow-hidden backdrop-blur-3xl group">
                       <div className="flex-1 text-center md:text-left space-y-6">
                          <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center justify-center md:justify-start gap-3"><RadarIcon size={14} className="text-fuchsia-400"/> READINESS BALANCE</p>
                          <div className="grid grid-cols-2 gap-4 pt-4">
                             {[ {l:'OPERATIONS', v:history.length}, {l:'GLOBAL SCORE', v:score+'%'} ].map((x,i)=>(
                                <div key={i} className="bg-black/50 p-6 rounded-3xl border border-white/5 shadow-inner hover:border-fuchsia-500/30 transition-all"><p className="text-[9px] font-black text-slate-500 mb-2 uppercase">{x.l}</p><p className="text-4xl font-black text-white font-mono tracking-tighter">{x.v}</p></div>
                             ))}
                          </div>
                       </div>
                       <div className="w-full md:w-[320px] h-[300px] mt-10 md:mt-0">
                    ...
                    {/* WIDGET 2: RATING */}
                    <motion.div variants={itemVariants} className="lg:col-span-5 bg-[#0a0a0f]/90 border border-white/10 rounded-[3rem] p-10 flex flex-col items-center justify-center text-center shadow-2xl relative backdrop-blur-3xl group overflow-hidden">
                       <div className={`p-10 rounded-[2.5rem] mb-8 border-2 shadow-2xl transform group-hover:scale-110 transition-all duration-700`} style={{ borderColor: `${readiness.color}30`, backgroundColor: `${readiness.color}05` }}>
                          <ShieldAlert size={80} style={{ color: readiness.color }} className="animate-pulse" />
                       </div>
                       <p className="text-[11px] font-black text-slate-500 tracking-widest mb-2 uppercase">CLASSIFICATION</p>
                 ...
                 {/* COMPACT TABLE (RE-SCALED & SEAMLESS) */}
                 <motion.div variants={itemVariants} className="bg-[#0a0a0f]/80 border border-white/5 rounded-[3rem] p-8 shadow-2xl relative overflow-hidden backdrop-blur-3xl">
                    <div className="relative z-10 bg-black/40 border border-white/10 rounded-[2.5rem] overflow-hidden shadow-inner">
                      <table className="w-full text-center border-collapse">
                        <thead>
                          <tr className="border-b-2 border-white/10 bg-white/[0.04]"><th className="py-7 text-[10px] font-black tracking-[0.6em] text-slate-400 uppercase">SESSION</th><th className="py-7 text-[10px] font-black tracking-[0.6em] text-slate-400 uppercase">OPERATIONAL SECTOR</th><th className="py-7 text-[10px] font-black tracking-[0.6em] text-slate-400 uppercase">ACCURACY</th><th className="py-7 text-[10px] font-black tracking-[0.6em] text-slate-400 uppercase">ACTION</th></tr>
                        </thead>
                        <tbody className="text-white font-bold">
                           {history.length === 0 ? (
                              <tr><td colSpan={4} className="py-32 text-slate-700 font-black text-sm uppercase tracking-[0.8em] animate-pulse">EMPTY DATABASE</td></tr>
                           ) : (
                              history.map((h, i) => {
                                const st = getReadinessData(h.score);
                                return (
                                  <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02] transition-all group">
                                    <td className="py-10 text-[13px] font-black font-mono text-fuchsia-400 opacity-60 group-hover:opacity-100">#0{history.length - i}</td>
                                    <td className="py-10 text-[13px] font-black uppercase tracking-widest">{h.domain_id}</td>
                                    <td className="py-10">
                                       <div className="flex flex-col items-center gap-3">
                                          <span className="text-2xl font-black font-mono tracking-tighter text-white">{h.score}%</span>
                                          <div className="w-24 h-1.5 bg-black rounded-full overflow-hidden border border-white/5"><div className={`h-full ${st.bg} shadow-[0_0_10px_currentColor] transition-all duration-1000`} style={{ width: `${h.score}%`, color: st.color }} /></div>
                                       </div>
                                    </td>
            ...
            {view === 'assessment' && (
              <motion.div key="assess-hub" variants={containerVariants} initial="hidden" animate="show" className="max-w-6xl mx-auto space-y-16">
                <motion.div variants={itemVariants} className="text-center space-y-4 pt-6"><h2 className="text-6xl font-black text-white tracking-tighter uppercase leading-none drop-shadow-2xl">Target Modules.</h2><p className="text-fuchsia-400 text-[12px] font-black tracking-[0.6em] uppercase">Select strategic sector for diagnostic protocol</p></motion.div>
            ...
            <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} className="relative w-full max-w-4xl max-h-[85vh] bg-[#070707] border border-white/10 rounded-[4rem] p-12 shadow-2xl flex flex-col overflow-hidden text-center">
               ...
               <div className="flex-1 overflow-y-auto pr-6 space-y-6 custom-scrollbar">
                 {detailModal.map((d: any, i: number) => (
                    <div key={i} className={`p-10 rounded-[3rem] border bg-black/60 flex flex-col items-center ${d.is_correct ? 'border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.1)]' : 'border-red-500/20 shadow-[0_0_15px_rgba(239,68,68,0.1)]'}`}>
