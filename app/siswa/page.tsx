...
import { 
  ...
  Lightbulb // <-- FIX: SUDAH DITAMBAHKAN DI SINI
} from 'lucide-react'
import {
...
export default function StudentPortal() {
  ...
  const submitAppFeedback = async () => {
    ...
    try {
      ...
      if (response.ok) { alert(`Transmission successful. Check Gmail.`); setAppFeedbackModal(false); setAppFeedbackForm({ category: 'AI ENHANCEMENT', message: '' }); } 
    } catch (e) { alert("Error."); } 
    ...
    <div className="flex h-screen w-full text-slate-100 overflow-hidden font-sans selection:bg-fuchsia-500/30">
      ...
      <motion.aside initial={false} animate={{ width: isSidebarCollapsed ? 80 : 250 }} className="h-screen bg-[#05050A]/95 backdrop-blur-3xl border-r border-white/10 flex flex-col z-[100] transition-all duration-500 shadow-2xl">
        <div className="h-20 px-6 flex items-center justify-between border-b border-white/10">
          ...
          <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="p-2 text-slate-400 hover:text-fuchsia-400 border border-white/5 rounded-lg transition-all">
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
        <header className="h-20 flex items-center justify-between px-10 border-b border-white/10 bg-black/40 backdrop-blur-xl">
            ...
            <div className="flex items-center gap-6 text-right"><div className="hidden sm:block"><p className="text-[11px] font-black text-white tracking-widest uppercase">{user.username}</p><p className="text-[10px] font-bold text-fuchsia-400 uppercase tracking-[0.2em] mt-1">OPERATIVE MODE</p></div><div className="w-11 h-11 bg-gradient-to-br from-indigo-500 to-fuchsia-600 rounded-full flex items-center justify-center border border-white/20 shadow-lg shadow-fuchsia-500/20"><User size={20} /></div></div>
        </header>
            ...
            {view === 'dashboard' && (
              <motion.div key="dash" variants={containerVariants} initial="hidden" animate="show" className="max-w-[1300px] mx-auto space-y-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-8 lg:p-10 rounded-[2.5rem] lg:rounded-[3rem] flex flex-col items-center justify-center text-center hover:border-fuchsia-500/40 transition-all shadow-2xl relative overflow-hidden group"><div className="absolute top-0 left-0 w-full h-1 bg-fuchsia-500 shadow-[0_0_15px_#d946ef] animate-scan opacity-0 group-hover:opacity-100" /><div className="w-16 lg:w-20 h-16 lg:h-20 bg-gradient-to-b from-fuchsia-600 to-indigo-900 rounded-3xl flex items-center justify-center text-white border border-white/10 mb-6 transform group-hover:scale-110 transition-all duration-500"><Fingerprint size={36} /></div><h4 className="text-xl lg:text-2xl font-black text-white tracking-widest uppercase leading-none">{user.username}</h4><p className="text-[9px] lg:text-[10px] text-fuchsia-400 font-black tracking-[0.4em] uppercase mt-4">{user.class_name || "NOT ASSIGNED"}</p></motion.div>
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-8 lg:p-10 rounded-[2.5rem] lg:rounded-[3rem] flex flex-col items-center justify-center hover:border-fuchsia-500/40 transition-all shadow-2xl group text-center">
                      ...
                      <div className={`px-8 py-1.5 rounded-full border border-white/10 text-[9px] font-black tracking-widest uppercase bg-black/40 ${readiness.text} shadow-lg shadow-black/50`}>{readiness.label}</div>
                   </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-[#020108]/96 backdrop-blur-[50px]">
             ...
             <motion.div initial={{ scale: 0.9, y: 40, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.9, y: 20, opacity: 0 }} transition={{ type: 'spring', damping: 25 }} className="w-full max-w-2xl bg-[#0a0a0f]/80 border border-white/10 rounded-[3rem] p-12 shadow-[0_0_80px_rgba(217,70,239,0.2)] relative overflow-hidden backdrop-blur-[100px]">
                ...
                <div className="space-y-10 relative z-10">
                   <div>
                      <label className="text-[11px] font-black text-fuchsia-400 uppercase tracking-widest mb-4 flex items-center gap-3"><Hexagon size={14}/> SELECT CATEGORY</label>
