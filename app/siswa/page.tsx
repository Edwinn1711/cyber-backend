  ...
  useEffect(() => {
    const handleInteraction = (e: MouseEvent | TouchEvent) => {
      ...
      const newParticles = Array.from({ length: 12 }).map((_, i) => ({
        id: Math.random(), x: clientX, y: clientY, angle: (Math.PI * 2 / 12) * i, velocity: Math.random() * 60 + 30
      }));
...
// --- 2. BACKGROUND COMPONENT (DEEP & SHARP) ---
...
export default function StudentPortal() {
  ...
  const radarData = useMemo(() => {
    ...
    return domains.map(d => {
      const entry = history.find(h => h.domain_id.toLowerCase().includes(d.toLowerCase()));
  ...
  return (
    <div className="flex h-screen w-full text-slate-200 overflow-hidden font-sans selection:bg-fuchsia-500/30">
      ...
      {/* --- SIDEBAR --- */}
      <motion.aside initial={false} animate={{ width: isSidebarCollapsed ? 80 : 260 }} className="h-screen bg-[#05050A]/95 backdrop-blur-3xl border-r border-white/5 flex flex-col z-[100] transition-all duration-500 shadow-2xl">
        ...
        <nav className="flex-1 px-4 py-8 space-y-3">
          {[ { id: 'dashboard', label: 'BERANDA', icon: LayoutGrid }, { id: 'assessment', label: 'ASSESSMENT', icon: Target }, { id: 'reports', label: 'REPORT', icon: FileText } ].map((item) => (
            <button key={item.id} onClick={() => setView(item.id)} className={`w-full flex items-center p-3.5 rounded-xl transition-all gap-4 ${view === item.id ? 'bg-fuchsia-600/10 text-fuchsia-400 border border-fuchsia-500/20 shadow-lg' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}>
              <item.icon size={18} /> {!isSidebarCollapsed && <span className="font-bold text-[9px] tracking-widest uppercase">{item.label}</span>}
            </button>
          ))}
        </nav>
        <div className="p-6 border-t border-white/5"><button onClick={() => router.push('/')} className="w-full flex items-center justify-center p-3 bg-red-950/20 text-red-500 border border-red-500/20 rounded-xl gap-4 font-black text-[9px] tracking-widest uppercase hover:bg-red-600 hover:text-white transition-all"><Power size={16} /> {!isSidebarCollapsed && "LOGOUT"}</button></div>
      </motion.aside>
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
        <header className="h-20 flex items-center justify-between px-10 border-b border-white/5 bg-black/30 backdrop-blur-xl">
            <div className="flex items-center gap-3 px-5 py-2 bg-white/5 border border-white/10 rounded-full shadow-inner"><div className="w-2.5 h-2.5 bg-emerald-400 rounded-full animate-pulse shadow-[0_0_15px_#34d399]" /><span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">GATEWAY SECURED</span></div>
            <div className="flex items-center gap-6 text-right"><div className="hidden sm:block"><p className="text-[11px] font-black text-white tracking-widest uppercase">{user.username}</p><p className="text-[9px] font-bold text-fuchsia-400 uppercase tracking-widest mt-1">OPERATIVE MODE</p></div><div className="w-10 h-10 bg-gradient-to-br from-violet-600 to-fuchsia-600 rounded-full flex items-center justify-center border border-white/20 shadow-lg shadow-fuchsia-500/20"><User size={20} /></div></div>
        </header>

        <main className="flex-1 overflow-y-auto no-scrollbar px-8 lg:px-12 py-8" ref={scrollRef}>
          <AnimatePresence mode="wait">
            
            {/* VIEW DASHBOARD (FIXED CENTERED) */}
            {view === 'dashboard' && (
              <motion.div key="dash" variants={containerVariants} initial="hidden" animate="show" className="max-w-[1300px] mx-auto space-y-12">
                <motion.div variants={itemVariants} className="text-center space-y-4 pt-4">
                  <div className="text-fuchsia-400 font-black text-[11px] tracking-[0.5em] uppercase flex items-center justify-center gap-3">{getGreeting()} OPERATIVE</div>
                  ...
                  <p className="text-slate-400 text-[12px] font-bold tracking-[0.3em] uppercase max-w-xl mx-auto">Cyber Readiness Index Protocol Gateway v2.1</p>
                </motion.div>

                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-10 rounded-[3rem] flex flex-col items-center justify-center text-center hover:border-fuchsia-500/40 transition-all shadow-2xl group">
                        <div className="w-20 h-20 bg-gradient-to-b from-fuchsia-600 to-indigo-900 rounded-3xl flex items-center justify-center text-white border border-white/10 mb-8 transform group-hover:scale-110 transition-all"><Fingerprint size={36} /></div>
                        <h4 className="text-2xl font-black text-white tracking-widest uppercase">{user.username}</h4>
                        <p className="text-[10px] text-fuchsia-400 font-bold uppercase mt-4 tracking-widest">{user.class_name}</p>
                   </motion.div>
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-10 rounded-[3rem] flex flex-col items-center justify-center hover:border-fuchsia-500/40 transition-all shadow-2xl group">
                      <div className="relative mb-6 flex justify-center w-full transform group-hover:scale-105 transition-all">
                        <svg className="w-36 h-36 transform -rotate-90">
                          ...
                          <motion.circle initial={{ strokeDasharray: "0 1000" }} animate={{ strokeDasharray: `${(score / 100) * 414} 1000` }} transition={{ duration: 2 }} cx="72" cy="72" r="66" stroke="#d946ef" strokeWidth="6" strokeLinecap="round" fill="transparent" style={{ filter: 'drop-shadow(0 0 10px rgba(217,70,239,0.8))' }} />
                        </svg>
                      ...
                      <div className={`px-8 py-1.5 rounded-full border border-fuchsia-500/30 text-[9px] font-black tracking-widest uppercase bg-black/40 ${readiness.text}`}>{readiness.label}</div>
                   </motion.div>
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-10 rounded-[3rem] hover:border-fuchsia-500/40 transition-all shadow-2xl flex flex-col justify-between group">
                      <p className="text-[10px] font-black text-slate-500 tracking-[0.4em] mb-10 uppercase flex items-center justify-center gap-3"><Activity size={14} className="text-fuchsia-500"/> SYSTEM INFRA</p>
                      <div className="space-y-5 font-mono text-[9px] tracking-[0.2em] text-slate-400 w-full px-4">
                        <div className="flex justify-between border-b border-white/5 pb-2"><span>HOST</span><span className="text-fuchsia-400">VERCEL</span></div>
                        <div className="flex justify-between border-b border-white/5 pb-2"><span>CRYPT</span><span className="text-emerald-500">AES-256</span></div>
                        <div className="flex justify-between"><span>CLOUD</span><span className="text-white">AIVEN DB</span></div>
                      </div>
                   </motion.div>
                   <motion.div variants={itemVariants} className="lg:col-span-3 bg-gradient-to-br from-[#0a0a14] to-[#020202] border border-white/10 p-10 rounded-[3rem] relative overflow-hidden flex flex-col lg:flex-row items-center justify-between gap-10 shadow-2xl group hover:border-fuchsia-500/30 transition-all duration-700">
                      <div className="flex items-center gap-10 z-10 text-center lg:text-left">
                        <div className="w-20 h-20 bg-black/80 rounded-3xl flex items-center justify-center shrink-0 border border-white/10 shadow-2xl group-hover:scale-110 group-hover:rotate-6 transition-all duration-500"><BellRing size={32} className="text-fuchsia-500 animate-bounce" /></div>
                        <div className="space-y-3"><div className="flex items-center justify-center lg:justify-start gap-4"><div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_15px_#10b981] animate-ping" /><span className="text-[10px] font-black tracking-[0.6em] text-emerald-400 uppercase">Mission Status: Active</span></div><h3 className="text-3xl font-black text-white uppercase tracking-tighter">System Readiness Test Active.</h3><p className="text-[11px] font-bold text-slate-400 tracking-widest max-w-2xl leading-relaxed uppercase">Initiate your cyber defense protocols now to validate personnel security.</p></div>
                      </div>
                      <button onClick={handleStartMissionClick} className="px-14 py-6 bg-white text-black rounded-3xl font-black text-[11px] tracking-[0.5em] hover:bg-fuchsia-600 hover:text-white transition-all shadow-2xl flex items-center gap-5 uppercase z-10">INITIALIZE ASSESSMENT <Zap size={18} /></button>
                   </motion.div>
                </div>
              </motion.div>
            )}

            {/* VIEW REPORTS - RESULTS (DEWA VERSION) */}
            {view === 'reports' && (
              <motion.div key="reports" variants={containerVariants} initial="hidden" animate="show" className="max-w-[1400px] mx-auto space-y-10">
                 
                 <motion.div variants={itemVariants} className="text-center space-y-4 pt-4">
                    <div className="text-fuchsia-400 font-black text-[12px] tracking-[0.6em] uppercase flex items-center justify-center gap-3"><TrendingUp size={16}/> PERFORMANCE ANALYTICS</div>
                    <h2 className="text-5xl lg:text-6xl font-black text-white tracking-widest uppercase drop-shadow-2xl">Mission Intelligence.</h2>
                    <p className="text-slate-500 text-[11px] font-bold tracking-[0.4em] uppercase max-w-xl mx-auto">Historical operative data record summary from the secure vaults.</p>
                 </motion.div>

                 {/* TOP WIDGETS GRID */}
                 <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    {/* WIDGET 1: ACCURACY RADAR (GOD TIER) */}
                    <motion.div variants={itemVariants} className="lg:col-span-7 bg-[#0a0a0f]/90 border border-white/10 rounded-[3rem] p-10 flex flex-col md:flex-row items-center justify-between shadow-2xl relative overflow-hidden backdrop-blur-3xl group">
                       <div className="absolute top-0 right-0 w-32 h-32 bg-fuchsia-600/5 blur-[80px]" />
                       <div className="flex-1 text-center md:text-left space-y-6">
                          <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center justify-center md:justify-start gap-3"><RadarIcon size={14} className="text-fuchsia-400"/> SECTOR READINESS BALANCE</p>
                          <div className="grid grid-cols-2 gap-4 pt-4">
                             {[ {l:'TOTAL SESSIONS', v:history.length}, {l:'GLOBAL SCORE', v:score+'%'} ].map((x,i)=>(
                                <div key={i} className="bg-black/50 p-6 rounded-3xl border border-white/5 shadow-inner">
                                   <p className="text-[9px] font-black text-slate-500 mb-2 uppercase">{x.l}</p>
                                   <p className="text-4xl font-black text-white font-mono tracking-tighter">{x.v}</p>
                                </div>
                             ))}
                          </div>
                       </div>
                       <div className="w-full md:w-[320px] h-[300px] mt-10 md:mt-0 relative">
                          <ResponsiveContainer width="100%" height="100%">
                             <RadarChart cx="50%" cy="50%" outerRadius="75%" data={radarData}>
                                <PolarGrid stroke="rgba(255,255,255,0.05)" />
                                <PolarAngleAxis dataKey="subject" tick={{ fill: '#94a3b8', fontSize: 9, fontWeight: 'bold' }} />
                                <Radar name="Score" dataKey="A" stroke="#d946ef" fill="#d946ef" fillOpacity={0.4} />
                                <Tooltip contentStyle={{ backgroundColor: '#000', borderRadius: '12px', border: '1px solid #333', fontSize: '10px' }} />
                             </RadarChart>
                          </ResponsiveContainer>
                       </div>
                    </motion.div>

                    {/* WIDGET 2: OVERALL CLASSIFICATION */}
                    <motion.div variants={itemVariants} className="lg:col-span-5 bg-[#0a0a0f]/90 border border-white/10 rounded-[3rem] p-10 flex flex-col items-center justify-center text-center shadow-2xl relative overflow-hidden backdrop-blur-3xl group">
                       <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-indigo-600/5 blur-[120px] -z-10" />
                       <div className={`p-8 rounded-[2.5rem] mb-8 border-2 shadow-2xl transform group-hover:scale-110 transition-all duration-700`} style={{ borderColor: `${readiness.color}30`, backgroundColor: `${readiness.color}05` }}>
                          <ShieldAlert size={80} style={{ color: readiness.color }} className="animate-pulse drop-shadow-[0_0_20px_rgba(217,70,239,0.5)]" />
                       </div>
                       <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest mb-2">PERSONNEL RATING</p>
                       <h3 className={`text-5xl font-black tracking-widest uppercase ${readiness.text}`} style={{ textShadow: `0 0 20px ${readiness.color}60` }}>{readiness.label}</h3>
                       <p className="text-[10px] text-slate-500 font-bold uppercase mt-6 tracking-widest">Based on {history.length} successful missions</p>
                    </motion.div>
                 </div>

                 {/* DATA TABLE (COMPACT DEWA VERSION) */}
                 <motion.div variants={itemVariants} className="bg-[#0a0a0f]/80 border border-white/5 rounded-[3rem] p-8 shadow-2xl relative overflow-hidden backdrop-blur-3xl">
                    <div className="absolute top-0 right-0 p-8 opacity-[0.02] text-8xl font-black tracking-[1em] pointer-events-none uppercase">LOGS</div>
                    <div className="flex items-center justify-between mb-8 px-6">
                       <div className="flex items-center gap-4"><div className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 shadow-inner"><Terminal size={20} className="text-fuchsia-400" /></div><span className="text-[14px] font-black text-white uppercase tracking-[0.4em]">Historical Mission Log</span></div>
                       <div className="bg-black/50 px-6 py-2.5 rounded-full border border-white/10 text-[9px] font-black tracking-widest text-slate-400 uppercase">Synchronized: <span className="text-emerald-400">Stable</span></div>
                    </div>

                    <div className="bg-black/40 border border-white/10 rounded-[2.5rem] overflow-hidden shadow-inner mx-2">
                      <table className="w-full text-center border-collapse">
                        <thead>
                          <tr className="border-b-2 border-white/10 bg-white/[0.04]">
                            <th className="py-6 text-[10px] font-black tracking-[0.5em] text-slate-400 uppercase">SESSION</th>
                            <th className="py-6 text-[10px] font-black tracking-[0.5em] text-slate-400 uppercase">OPERATIONAL SECTOR</th>
                            <th className="py-6 text-[10px] font-black tracking-[0.5em] text-slate-400 uppercase">METRIC ACCURACY</th>
                            <th className="py-6 text-[10px] font-black tracking-[0.5em] text-slate-400 uppercase">SYSTEM ACTION</th>
                          </tr>
                        </thead>
                        <tbody className="text-white font-bold">
                           {history.length === 0 ? (
                              <tr><td colSpan={4} className="py-24 text-slate-700 font-black text-sm uppercase tracking-[1em] animate-pulse">NO SYSTEM RECORDS DETECTED</td></tr>
                           ) : (
                              history.map((h, i) => {
                                const rd = getReadinessData(h.score);
                                return (
                                  <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02] transition-all group">
                                    <td className="py-8"><span className="text-[13px] font-black font-mono text-fuchsia-400 opacity-60 group-hover:opacity-100 transition-all tracking-tighter">ID#0{history.length - i}</span></td>
                                    <td className="py-8"><div className="flex flex-col items-center"><span className="text-[13px] font-black uppercase tracking-widest text-white leading-none">{h.domain_id}</span><span className="text-[9px] text-slate-600 mt-2 tracking-widest">ENCRYPTED SECTOR</span></div></td>
                                    <td className="py-8">
                                       <div className="flex flex-col items-center gap-3">
                                          <span className="text-2xl font-black font-mono tracking-tighter text-white">{h.score}%</span>
                                          <div className="w-24 h-1.5 bg-black rounded-full overflow-hidden border border-white/5"><div className={`h-full ${rd.bg} shadow-[0_0_10px_currentColor] transition-all duration-1000`} style={{ width: `${h.score}%`, color: rd.color }} /></div>
                                       </div>
                                    </td>
                                    <td className="py-8"><button onClick={() => setDetailModal(h.details || [])} className="mx-auto flex items-center gap-4 px-10 py-3.5 bg-white/5 border border-white/10 hover:bg-white text-slate-400 hover:text-black rounded-full text-[10px] font-black tracking-[0.5em] transition-all uppercase shadow-xl hover:shadow-white/10 group/btn transform hover:translate-y-[-2px]"><Eye size={18} className="group-hover/btn:animate-pulse" /> DOSSIER</button></td>
                                  </tr>
                                )
                              })
                           )}
              ...
              <motion.div key="assess-hub" variants={containerVariants} initial="hidden" animate="show" className="max-w-6xl mx-auto space-y-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                  {TACTICAL_DOMAINS.map((domain, i) => {
                    ...
                    return (
                      <motion.div key={i} variants={itemVariants} whileHover={{ y: -12, scale: 1.02 }} onClick={() => { setSelectedDomain(domain.id); setView('briefing'); }} className={`bg-[#0a0a14]/90 border-2 ${done ? 'border-fuchsia-500/50' : 'border-white/10'} p-12 rounded-[4rem] cursor-pointer group transition-all shadow-2xl relative overflow-hidden`}><div className="flex justify-between items-start mb-12"><div className="w-18 h-18 rounded-[2rem] flex items-center justify-center shadow-2xl" style={{ backgroundColor: `${domain.color}15`, border: `1px solid ${domain.color}40` }}><domain.icon size={32} style={{ color: domain.color }} /></div>{done && <div className="bg-fuchsia-600 px-5 py-2 rounded-full border border-white/10 shadow-lg"><span className="text-[10px] font-black text-white tracking-widest uppercase">Verified</span></div>}</div><h3 className="text-3xl font-black text-white mb-4 uppercase tracking-tighter leading-none">{domain.title}</h3><p className="text-[12px] text-slate-500 font-medium tracking-widest uppercase mb-12 leading-relaxed">{domain.desc}</p><div className="mt-auto pt-8 border-t-2 border-white/5 flex justify-end items-center"><span className="text-[11px] font-black tracking-[0.5em] text-fuchsia-400 group-hover:translate-x-3 transition-all uppercase">INITIALIZE <ArrowRight size={18} className="inline ml-4" /></span></div></motion.div>
                    );
                  })}
                </div>
              </motion.div>
            ...
            {/* Mission & Briefing (Untouched high-quality) */}
            {view === 'mission' && (
            ...
            {view === 'briefing' && (
              <motion.div key="briefing" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-4xl mx-auto flex flex-col items-center justify-center min-h-[75vh]">
                 ...
                 <motion.div className="flex flex-col items-center text-center space-y-10"><div className="px-8 py-3 rounded-full bg-emerald-500/10 border-2 border-emerald-400 text-emerald-400 text-[12px] font-black tracking-[0.7em] uppercase shadow-[0_0_30px_rgba(52,211,153,0.2)] animate-pulse">Uplink Confirmed</div><h2 className="text-6xl lg:text-7xl font-black text-white tracking-tighter uppercase leading-none drop-shadow-2xl">Transmission Received.</h2><p className="text-[16px] font-black text-white leading-relaxed tracking-[0.3em] uppercase max-w-2xl opacity-100">Halo <span className="text-fuchsia-400">{user.username}</span>, inisiasi kuis untuk topik <span className="text-fuchsia-400">{selectedDomain}</span> sekarang?</p></motion.div>
      ...
      <AnimatePresence>
        {detailModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[3000] flex items-center justify-center p-6 bg-black/98 backdrop-blur-3xl">
            <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} className="relative w-full max-w-4xl max-h-[85vh] bg-[#070707] border border-white/10 rounded-[4rem] p-12 shadow-2xl flex flex-col overflow-hidden text-center">
               ...
               <div className="flex justify-between items-center mb-10 shrink-0">
                  <div><h2 className="text-2xl font-black text-white tracking-widest uppercase flex items-center gap-5"><Laptop size={28} className="text-fuchsia-500"/> POST-MISSION ANALYSIS</h2></div>
                  <button onClick={() => setDetailModal(null)} className="p-3 bg-white/5 rounded-2xl text-slate-500 hover:text-red-500 border border-white/5"><X size={24} /></button>
               </div>
               <div className="flex-1 overflow-y-auto pr-6 space-y-6 custom-scrollbar">
                 {detailModal.map((d: any, i: number) => (
                    <div key={i} className={`p-10 rounded-[3rem] border bg-black/60 flex flex-col items-center ${d.is_correct ? 'border-emerald-500/20' : 'border-red-500/20 shadow-[0_0_30px_rgba(239,68,68,0.05)]'}`}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .bg-hud-grid { background-image: linear-gradient(to right, rgba(255, 255, 255, 0.05) 1px, transparent 1px), linear-gradient(to bottom, rgba(255, 255, 255, 0.05) 1px, transparent 1px); background-size: 50px 50px; transform: perspective(800px) rotateX(45deg) scale(2.5); transform-origin: top; animation: grid-move 10s linear infinite; }
