...
// --- 0. HELPER FUNCTIONS (FIXED POSITION) ---
...
export default function StudentPortal() {
  ...
  // --- CORE FUNCTIONS (FIXED: ALL PRESENT) ---
  ...
  const readinessInfo = useMemo(() => getReadinessData(score), [score]);
    ...
    <div className="flex h-screen w-full text-slate-100 overflow-hidden font-sans selection:bg-fuchsia-500/30">
      ...
      <motion.aside initial={false} animate={{ width: isSidebarCollapsed ? 80 : 250 }} className="h-screen bg-[#05050A]/95 backdrop-blur-3xl border-r border-white/10 flex flex-col z-[100] transition-all duration-500 shadow-2xl">
        ...
        <nav className="flex-1 px-4 py-8 space-y-3">
          ...
          <button onClick={() => setAppFeedbackModal(true)} className="w-full flex items-center p-3.5 rounded-xl text-slate-400 hover:text-fuchsia-400 hover:bg-white/5 transition-all gap-4">
            <Lightbulb size={18} /> {!isSidebarCollapsed && <span className="font-bold text-[10px] tracking-widest uppercase">FEEDBACK</span>}
          </button>
        </nav>
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
        <header className="h-20 flex items-center justify-between px-6 lg:px-10 border-b border-white/10 bg-black/40 backdrop-blur-xl">
            ...
            <div className="flex items-center gap-4 lg:gap-6 text-right"><div className="hidden sm:block"><p className="text-[11px] font-black text-white tracking-widest uppercase">{user.username}</p><p className="text-[10px] font-bold text-fuchsia-400 uppercase tracking-[0.2em] mt-1">OPERATIVE MODE</p></div><div className="w-11 h-11 bg-gradient-to-br from-indigo-500 to-fuchsia-600 rounded-full flex items-center justify-center border border-white/20 shadow-lg"><User size={20} /></div></div>
        </header>
          ...
          <AnimatePresence mode="wait">
            ...
            {view === 'dashboard' && (
              <motion.div key="dash" variants={containerVariants} initial="hidden" animate="show" className="max-w-[1300px] mx-auto space-y-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-8 lg:p-10 rounded-[2.5rem] lg:rounded-[3rem] flex flex-col items-center justify-center text-center hover:border-fuchsia-500/40 transition-all shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="w-16 lg:w-20 h-16 lg:h-20 bg-gradient-to-b from-fuchsia-600 to-indigo-900 rounded-3xl flex items-center justify-center text-white border border-white/10 mb-6 transform group-hover:scale-110 transition-all duration-500"><Fingerprint size={36} /></div>
                        <h4 className="text-xl lg:text-2xl font-black text-white tracking-widest uppercase leading-none">{user.username}</h4>
                        <p className="text-[9px] lg:text-[10px] text-fuchsia-400 font-black tracking-[0.4em] uppercase mt-4">{user.class_name || "NOT ASSIGNED"}</p>
                   </motion.div>
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-8 lg:p-10 rounded-[2.5rem] lg:rounded-[3rem] flex flex-col items-center justify-center hover:border-fuchsia-500/40 transition-all shadow-2xl group text-center">
                      <div className="relative mb-4 flex justify-center w-full transform group-hover:scale-105 transition-all">
                        <svg className="w-32 lg:w-40 h-32 lg:h-40 transform -rotate-90">
                          <circle cx="50%" cy="50%" r="42%" stroke="rgba(255,255,255,0.03)" strokeWidth="6" fill="transparent" />
                          <motion.circle initial={{ strokeDasharray: "0 1000" }} animate={{ strokeDasharray: `${(score / 100) * 452} 1000` }} transition={{ duration: 2.5, ease: "circOut" }} cx="50%" cy="50%" r="42%" stroke="#d946ef" strokeWidth="8" strokeLinecap="round" fill="transparent" style={{ filter: 'drop-shadow(0 0 15px rgba(217,70,239,0.8))' }} />
                        </svg>
                        <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-4xl lg:text-6xl font-black text-white tracking-tighter">{score}</span><span className="text-[7px] lg:text-[8px] font-black text-slate-500 uppercase mt-1">AVG SCORE</span></div>
                      </div>
                      <div className={`px-8 py-1.5 rounded-full border border-white/10 text-[9px] font-black tracking-widest uppercase bg-black/40 ${readinessInfo.text} shadow-lg shadow-black/50`}>{readinessInfo.label}</div>
                   </motion.div>
                   <motion.div variants={itemVariants} className="bg-[#0a0a0f]/90 border border-white/5 p-8 lg:p-10 rounded-[2.5rem] lg:rounded-[3rem] hover:border-fuchsia-500/40 transition-all shadow-2xl flex flex-col justify-between group text-center"><p className="text-[9px] lg:text-[10px] font-black text-slate-500 tracking-[0.5em] mb-10 uppercase flex items-center justify-center gap-3"><Activity size={16} className="text-fuchsia-500 animate-pulse"/> SYSTEM ANALYTICS</p>
                   ...
                   {/* --- BROADCAST CENTER (FIXED RESPONSIVE) --- */}
              ...
              <motion.div key="assess-hub" variants={containerVariants} initial="hidden" animate="show" className="max-w-6xl mx-auto space-y-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10 pb-20">
                  {TACTICAL_DOMAINS.map((domain, i) => {
                    ...
                    return (<motion.div key={i} variants={itemVariants} whileHover={{ y: -10, scale: 1.02 }} onClick={() => { setSelectedDomain(domain.id); setView('briefing'); }} className={`bg-[#0a0a14]/90 border-2 ${done ? 'border-fuchsia-500/50 shadow-[0_0_30px_rgba(217,70,239,0.15)]' : 'border-white/10'} p-10 lg:p-12 rounded-[3.5rem] lg:rounded-[4rem] cursor-pointer group transition-all duration-500 shadow-2xl relative overflow-hidden`}><div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" /><div className="flex justify-between items-start mb-10"><div className="w-16 lg:w-18 h-16 lg:h-18 rounded-[1.5rem] lg:rounded-[2rem] flex items-center justify-center shadow-2xl" style={{ backgroundColor: `${domain.color}15`, border: `2px solid ${domain.color}40` }}><domain.icon size={30} style={{ color: domain.color }} /></div>{done && <div className="bg-fuchsia-600 px-4 py-1.5 rounded-full border border-white/10 shadow-lg"><span className="text-[10px] font-black text-white tracking-widest uppercase">Verified</span></div>}</div><h3 className="text-2xl lg:text-3xl font-black text-white mb-4 uppercase tracking-tighter leading-none">{domain.title}</h3><p className="text-[11px] lg:text-[12px] text-slate-400 font-medium tracking-widest uppercase mb-10 leading-relaxed opacity-80">{domain.desc}</p><div className="mt-auto pt-8 border-t-2 border-white/5 flex justify-end items-center"><span className="text-[10px] lg:text-[11px] font-black tracking-[0.5em] text-fuchsia-400 group-hover:translate-x-3 transition-all uppercase">INITIALIZE <ArrowRight size={16} className="inline ml-3" /></span></div></motion.div>);
                  })}
                </div>
              </motion.div>
            )}
            ...
            {view === 'mission' && (
              <motion.div key="mission" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto space-y-12 pb-44 text-center lg:text-left">
                <header className="flex justify-between items-end border-b-2 border-white/10 pb-10 pt-6"><div className="space-y-4"><button onClick={() => setView('assessment')} className="text-[11px] font-black text-slate-400 hover:text-red-400 flex items-center gap-4 tracking-widest uppercase transition-all"><ArrowLeft size={20} /> ABORT SESSION</button><h2 className="text-4xl lg:text-6xl font-black text-white tracking-tighter uppercase leading-none">PHASE 0{currentStep}</h2><p className="text-fuchsia-500 font-black tracking-[0.6em] text-[12px] uppercase">PROTOCOL: {selectedDomain}</p></div><div className="text-[100px] lg:text-[140px] font-black text-white/5 font-mono leading-none tracking-tighter">0{currentStep}</div></header>
                <div className="space-y-10">{currentStepQs.map((q) => (<div key={q.id} className="p-10 lg:p-14 rounded-[3.5rem] lg:rounded-[4rem] bg-[#1a1a2e]/90 border-2 border-white/10 space-y-12 shadow-2xl relative overflow-hidden transition-all hover:border-fuchsia-500/40"><div className="absolute top-0 left-0 w-2 lg:w-2.5 h-full bg-fuchsia-600 shadow-[0_0_20px_#d946ef]" /><h4 className="text-2xl lg:text-3xl font-medium text-white leading-snug tracking-tight">"{q.text}"</h4><div className="grid grid-cols-1 gap-6">{q.options.slice(0, 4).map((opt: any, i: number) => (<button key={i} onClick={() => setAns({...ans, [q.id]: {score: opt.score, text: opt.text}})} className={`p-8 lg:p-10 rounded-[2.2rem] lg:rounded-[2.5rem] text-left transition-all border-2 text-[11px] lg:text-[12px] font-black tracking-[0.1em] uppercase flex items-center gap-8 ${ans[q.id]?.text === opt.text ? 'bg-fuchsia-600/20 border-fuchsia-500 text-white shadow-2xl' : 'bg-black/60 border-white/10 text-slate-400 hover:border-white/30'}`}><div className={`w-5 lg:w-6 h-5 lg:h-6 rounded-full border-[4px] flex items-center justify-center shrink-0 ${ans[q.id]?.text === opt.text ? 'border-fuchsia-400 shadow-[0_0_10px_#d946ef]' : 'border-slate-800'}`}>{ans[q.id]?.text === opt.text && <div className="w-2 lg:w-2.5 h-2 lg:h-2.5 bg-fuchsia-400 rounded-full shadow-[0_0_15px_#d946ef]" />}</div><span>{opt.text}</span></button>))}</div></div>))}</div>
                <div className="flex gap-6 pt-12">{currentStep > 1 && <button onClick={() => setCurrentStep(p => p - 1)} className="px-12 py-6 bg-black border-2 border-white/10 text-slate-400 rounded-3xl font-black text-[11px] uppercase tracking-widest hover:bg-white/5 transition-all">PREVIOUS</button>}{currentStep < maxStep ? <button disabled={!isStepComplete} onClick={() => setCurrentStep(p => p + 1)} className={`flex-1 py-6 rounded-3xl font-black text-[11px] tracking-widest transition-all uppercase ${isStepComplete ? 'bg-fuchsia-600 text-white shadow-2xl' : 'bg-white/5 text-slate-800 cursor-not-allowed'}`}>CONTINUE</button> : <button disabled={!isStepComplete || loading} onClick={executeUplink} className={`flex-1 py-6 rounded-3xl font-black text-[11px] tracking-widest transition-all uppercase flex items-center justify-center gap-6 ${isStepComplete ? 'bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white shadow-2xl' : 'bg-white/5 text-slate-800 cursor-not-allowed'}`}>{submitStatus} <Zap size={22}/></button>}</div>
              </motion.div>
            )}
            ...
            {view === 'briefing' && (
              <motion.div key="briefing" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-4xl mx-auto flex flex-col items-center justify-center min-h-[75vh]">
                 <div className="mb-20 lg:mb-24"><RobotFace size={40} ringSize="w-56 lg:w-64 h-56 lg:h-64" coreSize="w-36 lg:w-40 h-36 lg:h-40" /></div>
                 <motion.div className="flex flex-col items-center text-center space-y-8 lg:space-y-10 px-4"><div className="px-6 py-2.5 rounded-full bg-emerald-500/10 border-2 border-emerald-400 text-emerald-400 text-[11px] font-black tracking-[0.7em] uppercase shadow-[0_0_30px_rgba(52,211,153,0.2)] animate-pulse">Uplink Confirmed</div><h2 className="text-5xl lg:text-7xl font-black text-white tracking-tighter uppercase leading-none drop-shadow-2xl text-center">Transmission Received.</h2><p className="text-[14px] lg:text-[16px] font-black text-white leading-relaxed tracking-[0.2em] uppercase max-w-2xl opacity-100 mx-auto">Halo <span className="text-fuchsia-400">{user.username}</span>, inisiasi kuis untuk topik <span className="text-fuchsia-400">{selectedDomain}</span> sekarang?</p></motion.div>
             ...
             <motion.div initial={{ scale: 0.9, y: 40, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.9, y: 20, opacity: 0 }} transition={{ type: 'spring', damping: 25 }} className="w-full max-w-2xl bg-[#0a0a0f]/80 border border-white/10 rounded-[3rem] p-12 shadow-[0_0_80px_rgba(217,70,239,0.2)] relative overflow-hidden backdrop-blur-[100px]">
                ...
                <div className="space-y-10 relative z-10">
                   <div>
                      ...
                      <select value={appFeedbackForm.category} onChange={(e) => setAppFeedbackForm({...appFeedbackForm, category: e.target.value})} className="w-full bg-black/70 border border-white/10 rounded-[2rem] p-5 text-[12px] font-black text-white uppercase outline-none focus:border-fuchsia-500 focus:shadow-[0_0_30px_rgba(217,70,239,0.2)] transition-all appearance-none cursor-pointer"><option value="AI ENHANCEMENT">🤖 AI SYSTEM</option><option value="UI/UX MODERNIZATION">🎨 UI / UX</option><option value="SECURITY EXPANSION">🛡️ SECURITY</option><option value="REAL-TIME ANALYTICS">📊 ANALYTICS</option><option value="SYSTEM PERFORMANCE">⚡ PERFORMANCE</option><option value="MOBILE INTEGRATION">📱 MOBILE</option><option value="BUG REPORT">⚠️ BUG REPORT</option><option value="OTHER">🌐 OTHER</option></select>
                   </div>
                   <div>
      ...
      <AnimatePresence>
        {detailModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[3000] flex items-center justify-center p-6 bg-black/98 backdrop-blur-3xl text-center">
            <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} className="relative w-full max-w-4xl max-h-[85vh] bg-[#070707] border border-white/10 rounded-[4rem] p-12 shadow-2xl flex flex-col overflow-hidden">
