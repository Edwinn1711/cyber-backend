...
export default function DashboardGuruZenith() {
  ...
  // =========================================================================
  // DATA ENGINE (DIAMBIL DARI DATABASE ASLI)
  // =========================================================================
  ...
  const submitAppFeedback = async () => {
    ...
    try {
      ...
      if (response.ok) { alert(`Feedback Transmitted.`); setAppFeedbackModal(false); setAppFeedbackForm({ category: 'AI ENHANCEMENT', message: '' }); } 
    } catch (error) { console.error(error); } 
  ...
  return (
    <div className="flex h-screen w-full bg-[#020108] text-slate-200 overflow-hidden font-sans selection:bg-fuchsia-500/30 text-sm">
            ...
            <motion.div initial={{ scale: 0.95, y: 15 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} className="relative w-full max-w-4xl bg-[#0a0a0f]/95 border border-white/10 rounded-[2rem] p-8 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden backdrop-blur-xl">
               ...
               <div className="flex justify-between items-start mb-6 shrink-0 relative z-10">
                  <div className="flex items-center gap-6">
                     ...
                     <div><p className="text-[10px] font-bold text-fuchsia-400 tracking-widest uppercase mb-1">STUDENT EVALUATION</p><h2 className="text-3xl font-black text-white uppercase">{feedbackModal.username}</h2><p className="text-xs font-semibold text-slate-400 mt-1 uppercase">{feedbackModal.class_name} • {feedbackModal.testCount} TOTAL ATTEMPTS</p></div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {Object.entries(feedbackModal.scores).map(([k, v]: any, idx) => (
                       <div key={idx} className="bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col items-center">
                          <p className="text-[10px] text-slate-400 font-bold uppercase mb-2">{k}</p>
                          <p className={`text-4xl font-black ${getScoreData(v).text}`}>{v !== null ? v : '--'}</p>
                       </div>
                    ))}
                  </div>
                  <div className="space-y-3">
                    <h3 className="text-xs font-bold text-slate-300 flex items-center gap-2 uppercase tracking-wider mb-3"><Database size={16} /> INDIVIDUAL AUDIT LOGS</h3>
               ...
               <div className="pt-6 shrink-0 relative z-10">
                  <button onClick={submitStudentFeedback} className="w-full py-4 bg-white text-black hover:bg-fuchsia-500 hover:text-white rounded-2xl font-black text-sm tracking-widest flex items-center justify-center gap-3 transition-all uppercase">SEND FEEDBACK <Send size={16} /></button>
               </div>
            </motion.div>
          </motion.div>
        )}
      ...
      {/* MODAL SYSTEM FEEDBACK - ULTRA LUXURY VIOLET/FUCHSIA */}
      <AnimatePresence>
        {appFeedbackModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-[#020108]/95 backdrop-blur-[40px]">
             ...
             <motion.div initial={{ scale: 0.9, y: 30, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.9, y: 20, opacity: 0 }} className="w-full max-w-2xl bg-[#0a0a0f]/80 border border-white/10 rounded-[3rem] p-10 shadow-[0_0_60px_rgba(217,70,239,0.15)] relative overflow-hidden backdrop-blur-3xl">
                ...
                <div className="space-y-8 relative z-10">
                   ...
                   <button onClick={submitAppFeedback} disabled={isSendingFeedback} className="w-full py-5 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white rounded-[1.5rem] font-black text-[11px] tracking-[0.5em] uppercase transition-all flex items-center justify-center gap-5 shadow-2xl">{isSendingFeedback ? "TRANSMITTING..." : "EXECUTE TRANSMISSION"} <Send size={18}/></button>
                </div>
             </motion.div>
          </motion.div>
        )}
      ...
      <motion.aside animate={{ width: isSidebarCollapsed ? 80 : 280 }} className="h-screen bg-[#05050A]/80 backdrop-blur-2xl border-r border-white/5 flex flex-col z-[100] shadow-2xl">
        <div className="h-20 px-6 flex items-center justify-between border-b border-white/5">
          {!isSidebarCollapsed && <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-fuchsia-600 flex items-center justify-center shadow-lg"><LayoutDashboard size={20} className="text-white" /></div><span className="font-black text-white uppercase tracking-widest text-[11px]">INSTRUCTOR</span></div>}
        ...
        <nav className="flex-1 px-4 py-8 space-y-3">
          <button onClick={() => setView('dashboard')} className={`w-full flex items-center p-4 rounded-xl transition-all gap-4 ${view === 'dashboard' ? 'bg-fuchsia-600/10 text-fuchsia-400 border border-fuchsia-500/20 shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}><Activity size={22} />{!isSidebarCollapsed && <span className="font-bold text-xs tracking-widest uppercase">DASHBOARD</span>}</button>
          <button onClick={() => setAppFeedbackModal(true)} className="w-full flex items-center p-4 rounded-xl text-slate-400 hover:text-fuchsia-400 hover:bg-fuchsia-600/10 transition-all gap-4 group"><Lightbulb size={22} />{!isSidebarCollapsed && <span className="font-bold text-xs tracking-widest uppercase">FEEDBACK</span>}</button>
        </nav>
        <div className="p-6 border-t border-white/5"><button onClick={() => { localStorage.removeItem('user'); router.push('/'); }} className="w-full flex items-center justify-center p-3.5 bg-red-500/10 text-red-400 rounded-xl gap-3 font-bold text-xs tracking-widest uppercase transition-all"><LogOut size={18} /> {!isSidebarCollapsed && "LOGOUT"}</button></div>
      </motion.aside>
            ...
            {view === 'dashboard' && (
              <motion.div key="analytics" {...(portalTransition as any)} className="max-w-[1400px] w-full mx-auto space-y-10 pb-20">
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
                  <div><div className="text-fuchsia-400 font-bold text-[11px] tracking-widest mb-2 uppercase flex items-center gap-2"><Hexagon size={14} /> LIVE MONITORING</div><h1 className="text-4xl lg:text-5xl font-black text-white tracking-tight uppercase">WELCOME, <span className="text-fuchsia-400">{user.username}</span>.</h1><p className="text-slate-400 mt-2 font-medium text-sm">Overall readiness overview from cloud databanks.</p></div>
                  <button onClick={fetchData} className="px-6 py-3 bg-white/5 border border-white/10 text-white rounded-full font-bold text-xs hover:bg-white hover:text-black transition-all flex items-center gap-3 uppercase shadow-lg"><RefreshCcw size={14} className={loading ? "animate-spin text-fuchsia-400" : ""}/> REFRESH</button>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 auto-rows-[minmax(160px,auto)]">
                   <motion.div whileHover={{ y: -4 }} className="lg:col-span-4 lg:row-span-2 bg-[#0a0a0f]/60 border border-white/10 rounded-[2rem] p-8 shadow-xl flex flex-col justify-between">
                      ...
                      <div className="mt-8 bg-white/5 p-4 rounded-xl flex justify-between uppercase text-[10px] font-bold"><span className="text-slate-500">TOTAL STUDENTS</span><span className="text-white">{participationStats.total}</span></div>
                   </motion.div>
                   <motion.div whileHover={{ y: -4 }} className="lg:col-span-4 lg:row-span-2 bg-[#0a0a0f]/60 border border-white/10 rounded-[2rem] p-8 flex flex-col items-center shadow-xl">
                      ...
                      <div className="flex gap-4 mt-2">{readinessDistribution.map((item, idx) => (<div key={idx} className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: item.color }} /><span className="text-[9px] font-bold text-slate-300 uppercase">{item.name}</span></div>))}</div>
                   </motion.div>
                   <motion.div whileHover={{ y: -4 }} className="lg:col-span-4 lg:row-span-2 bg-[#0a0a0f]/60 border border-white/10 rounded-[2rem] p-8 shadow-xl h-[280px]">
                ...
                <div className="mt-16 space-y-8 pb-32">
                   <div className="flex flex-col xl:flex-row justify-between items-end gap-6 mb-8">
                      <div><h2 className="text-3xl font-black text-white tracking-tighter uppercase flex items-center gap-4">OPERATIVE DOSSIERS <Database className="text-fuchsia-400" size={24} /></h2><p className="text-slate-500 font-bold text-[10px] tracking-[0.2em] uppercase mt-2">Authenticated operatives logs.</p></div>
                      <div className="flex gap-4 w-full xl:w-auto">
                         <div className="relative flex-1 xl:w-80"><Search size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-fuchsia-400" /><input type="text" placeholder="SEARCH IDENTITY..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-black/90 border border-white/10 rounded-full py-4 pl-12 pr-6 text-[10px] font-bold text-white outline-none focus:border-fuchsia-500 transition-all placeholder:text-slate-700" /></div>
                   ...
                   <div className="flex flex-col gap-5">
                      <AnimatePresence>
                         {filteredReports.map((r, idx) => {
                            ...
                            return (
                               <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.03 }} key={idx} className="flex items-center justify-between bg-[#07070B]/70 backdrop-blur-3xl p-6 rounded-[2.5rem] border border-white/5 hover:border-fuchsia-500/30 transition-all group shadow-2xl relative overflow-hidden">
                                  <div className="absolute left-0 top-0 w-1.5 h-full opacity-30" style={{ backgroundColor: status.color }} />
                                  <div className="flex items-center gap-6"><div className="w-16 h-16 rounded-full bg-black border border-white/10 flex items-center justify-center relative shadow-inner"><User size={24} className="text-slate-500 group-hover:text-white" /><div className={`absolute bottom-0 right-0 w-4 h-4 rounded-full border-2 border-[#07070B] ${status.bg} shadow-[0_0_15px_currentColor]`} style={{ color: status.color }} /></div><div><p className="font-black text-xl text-white tracking-tight uppercase group-hover:text-fuchsia-400 transition-colors">{r.username}</p><p className="text-[10px] text-slate-500 font-black tracking-widest uppercase mt-1">{r.class_name}</p></div></div>
                                  <div className="flex items-center gap-8">
                                     <div className="hidden lg:grid grid-cols-3 gap-6">
                                        <div className="text-center group/score"><p className="text-[8px] font-black text-slate-500 uppercase mb-1 tracking-widest group-hover/score:text-fuchsia-400 transition-colors">SOC</p><p className="text-lg font-black text-fuchsia-400" style={{textShadow:'0 0 10px #d946ef60'}}>{r.scores.social ?? '--'}</p></div>
                                        <div className="text-center group/score"><p className="text-[8px] font-black text-slate-500 uppercase mb-1 tracking-widest group-hover/score:text-red-400 transition-colors">MAL</p><p className="text-lg font-black text-red-500" style={{textShadow:'0 0 10px #ef444460'}}>{r.scores.malware ?? '--'}</p></div>
                                        <div className="text-center group/score"><p className="text-[8px] font-black text-slate-500 uppercase mb-1 tracking-widest group-hover/score:text-blue-400 transition-colors">PHI</p><p className="text-lg font-black text-blue-500" style={{textShadow:'0 0 10px #3b82f660'}}>{r.scores.phish ?? '--'}</p></div>
                                     </div>
