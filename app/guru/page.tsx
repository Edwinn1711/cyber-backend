...
// =========================================================================
// 1. CLICK EFFECT - TETAP DI PERTAHANKAN
// =========================================================================
...
// =========================================================================
// 2. BACKGROUND ENGINE MEGAH - TETAP DI PERTAHANKAN
// =========================================================================
...
export default function DashboardGuruZenith() {
  ...
  // =========================================================================
  // DATA ENGINE - HANYA MENGAMBIL DATA ASLI (DUMMY DIHAPUS)
  // =========================================================================
  const fetchData = async () => {
    ...
    try {
      ...
      // HANYA PROSES JIKA ADA DATA DARI DATABASE
      if (dataArray.length > 0) {
        ...
        // JIKA KOSONG, BIARKAN KOSONG (JANGAN PANGGIL DUMMY)
        setReports([]);
      }

    } catch (e) { 
      console.error("Gagal mengambil data dari database cloud:", e);
      setReports([]); 
    } 
  ...
  const handleDeleteLog = async (logId: string | number) => {
    if (!logId) { alert("Error: Log ID not found."); return; }
    if (!window.confirm("SYSTEM WARNING: Are you sure you want to permanently delete this record?")) return;
    try {
  ...
  const submitStudentFeedback = () => {
    ...
    alert(`Feedback sent to: ${feedbackModal.username}`);
    setFeedbackModal(null); setFeedbackText("");
  };

  const submitAppFeedback = async () => {
    if(!appFeedbackForm.message.trim()) { alert("Please enter message."); return; }
    setIsSendingFeedback(true);
    try {
      const response = await fetch("https://formsubmit.co/ajax/7877d067ab806b68d3ae1ab4eb99ed6c", {
        method: "POST", headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        body: JSON.stringify({ _subject: `FEEDBACK: ${appFeedbackForm.category}`, Pengirim: user.username, Pesan: appFeedbackForm.message })
      });
  ...
  return (
    <div className="flex h-screen w-full bg-[#020108] text-slate-200 overflow-hidden font-sans selection:bg-fuchsia-500/30 text-sm">
            ...
            <motion.div initial={{ scale: 0.95, y: 15 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} className="relative w-full max-w-4xl bg-[#0a0a0f]/95 border border-white/10 rounded-[2rem] p-8 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden backdrop-blur-xl">
               ...
               <div className="flex justify-between items-start mb-6 shrink-0 relative z-10">
                  <div className="flex items-center gap-6">
                     ...
                     <div>
                        <p className="text-[10px] font-bold text-fuchsia-400 tracking-widest uppercase mb-1">STUDENT EVALUATION</p>
                        <h2 className="text-3xl font-black text-white uppercase">{feedbackModal.username}</h2>
                        <p className="text-xs font-semibold text-slate-400 uppercase">{feedbackModal.class_name}</p>
                     </div>
                  </div>
               ...
               <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar relative z-10 space-y-6">
                 <div className="grid grid-cols-3 gap-4">
                    {[
                      { l: 'SOC. ENG', s: feedbackModal.scores.social, a: feedbackModal.attempts.social, color: '#d946ef' },
                      { l: 'MALWARE', s: feedbackModal.scores.malware, a: feedbackModal.attempts.malware, color: '#ef4444' },
                      { l: 'PHISHING', s: feedbackModal.scores.phish, a: feedbackModal.attempts.phish, color: '#3b82f6' }
                    ].map((dom, idx) => (
                       <div key={idx} className="bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col items-center">
                          ...
                          <p className={`text-4xl font-black ${getScoreData(dom.s).text}`}>{dom.s !== null ? dom.s : '--'}</p>
                 ...
                 <div className="space-y-3">
                    <h3 className="text-xs font-bold text-slate-300 flex items-center gap-2 uppercase tracking-wider"><Database size={16} className="text-indigo-400"/> AUDIT LOGS</h3>
                    {feedbackModal.history && feedbackModal.history.length > 0 ? (
                      feedbackModal.history.map((log: any, i: number) => (
                        <div key={i} className="flex items-center justify-between p-4 bg-black/40 border border-white/5 rounded-xl">
                          <div className="flex flex-col">
                             <p className="text-xs font-bold text-white uppercase">AUDIT {i+1}</p>
                             <p className="text-[10px] text-slate-500 uppercase mt-1">{log.parsedDomain} | SCORE: <span className={getScoreData(log.parsedScore).text}>{log.parsedScore}</span></p>
                          </div>
                      ...
                      <div className="p-6 text-center border border-dashed border-white/10 rounded-xl bg-white/5 text-slate-500 uppercase text-[10px] font-bold tracking-widest">No Logs Found</div>
                    )}
                 </div>

                 <div className="space-y-3">
                    <label className="text-xs font-bold text-slate-300 flex items-center gap-2 uppercase tracking-wider"><MessageSquare size={16} className="text-fuchsia-400"/> INSTRUCTOR FEEDBACK</label>
                    <textarea value={feedbackText} onChange={(e) => setFeedbackText(e.target.value)} placeholder="Enter guidance here..." className="w-full h-24 bg-black/40 border border-white/10 rounded-xl p-4 text-white focus:border-fuchsia-500 outline-none text-sm resize-none" />
                 </div>
               </div>

               <div className="pt-6 shrink-0 relative z-10">
                  <button onClick={submitStudentFeedback} className="w-full py-4 bg-white text-black hover:bg-fuchsia-500 hover:text-white rounded-2xl font-black text-sm tracking-widest flex items-center justify-center gap-3 transition-all uppercase">
      ...
      {/* --- SIDEBAR --- */}
      <motion.aside animate={{ width: isSidebarCollapsed ? 80 : 280 }} className="h-screen bg-[#05050A]/80 backdrop-blur-2xl border-r border-white/5 flex flex-col z-[100] transition-all duration-500 shadow-2xl">
        <div className="h-20 px-6 flex items-center justify-between border-b border-white/5">
          {!isSidebarCollapsed && (
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-fuchsia-600 flex items-center justify-center shadow-lg"><LayoutDashboard size={20} className="text-white" /></div>
              <span className="font-black tracking-widest text-sm text-white uppercase">INSTRUCTOR</span>
            </div>
          )}
          <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="p-2 bg-white/5 hover:bg-white/10 text-slate-400 rounded-lg mx-auto"><ChevronLeft size={18} className={isSidebarCollapsed ? "rotate-180" : ""} /></button>
        </div>
        ...
        <nav className="flex-1 px-4 py-8 space-y-3">
          <button onClick={() => setView('dashboard')} className={`w-full flex items-center p-4 rounded-xl transition-all gap-4 ${view === 'dashboard' ? 'bg-fuchsia-600/10 text-fuchsia-400 border border-fuchsia-500/20 shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
             ...
             {!isSidebarCollapsed && <span className="font-bold text-xs tracking-widest uppercase">DASHBOARD</span>}
          </button>
          <button onClick={() => setAppFeedbackModal(true)} className="w-full flex items-center p-4 rounded-xl text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/10 transition-all gap-4">
             <Lightbulb size={22} />
        ...
        <header className="h-20 flex items-center justify-between px-8 border-b border-white/5 bg-black/20 backdrop-blur-md">
            ...
            <div className="flex items-center gap-5">
                <div className="text-right hidden sm:block">
                    ...
                    <p className="text-[10px] font-semibold text-fuchsia-400 uppercase">INSTRUCTOR ACCESS</p>
                </div>
                <div className="w-11 h-11 bg-fuchsia-600 rounded-full flex items-center justify-center border border-white/10"><User size={20} /></div>
            </div>
        </header>
          ...
          <motion.div key="analytics" {...portalTransition} className="max-w-[1400px] w-full mx-auto space-y-10 pb-20">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
              <div>
                 <div className="text-fuchsia-400 font-bold text-[11px] tracking-widest flex items-center gap-2 mb-2 uppercase">
                   <Hexagon size={14} /> ACTIVE MONITORING
                 </div>
                 ...
                 <p className="text-slate-400 mt-2 font-medium text-sm uppercase tracking-wider">Fleet readiness overview from secure databanks.</p>
              </div>
              <button onClick={fetchData} className="px-6 py-3 bg-white/5 border border-white/10 text-white rounded-full font-bold text-xs hover:bg-white hover:text-black transition-all flex items-center gap-3 uppercase shadow-lg">
                 <RefreshCcw size={14} className={loading ? "animate-spin text-fuchsia-400" : ""}/> REFRESH
              </button>
            </div>

            {/* ANALYTICS GRID */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
               <motion.div whileHover={{ y: -4 }} className="bg-[#0a0a0f]/60 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 shadow-xl">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">PARTICIPATION</p>
                  <p className="text-5xl font-black text-white mb-2">{participationStats.percentage}%</p>
                  <div className="w-full h-2 bg-black rounded-full overflow-hidden border border-white/5">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${participationStats.percentage}%` }} transition={{ duration: 1.5 }} className="h-full bg-fuchsia-500 shadow-[0_0_10px_#d946ef]" />
                  </div>
                  <p className="mt-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">{participationStats.active} / {participationStats.total} TOTAL STUDENTS</p>
               </motion.div>

               <motion.div whileHover={{ y: -4 }} className="bg-[#0a0a0f]/60 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 flex flex-col items-center shadow-xl">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 w-full text-left">FLEET STATUS</p>
                  <div className="relative w-full h-[160px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={readinessDistribution} innerRadius={60} outerRadius={75} dataKey="value" stroke="none">
                          {readinessDistribution.map((entry, index) => (<Cell key={index} fill={entry.color} />))}
                        </Pie>
                      </PieChart>
                    </ResponsiveContainer>
                    <div className="absolute inset-0 flex items-center justify-center font-black text-2xl text-white">{participationStats.active}</div>
                  </div>
                  <div className="flex gap-4 mt-2">
                    {readinessDistribution.map((item, i) => (
                      <div key={i} className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full" style={{backgroundColor: item.color}} /><span className="text-[9px] font-bold text-slate-400 uppercase">{item.name}</span></div>
                    ))}
                  </div>
               </motion.div>

               <motion.div whileHover={{ y: -4 }} className="bg-[#0a0a0f]/60 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 shadow-xl h-[260px]">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6">DOMAIN MASTERY</p>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={domainAverages} margin={{ left: -30 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(255,255,255,0.05)" />
                      <XAxis dataKey="domain" tick={{ fill: '#94a3b8', fontSize: 9, fontWeight: 'bold' }} axisLine={false} tickLine={false} />
                      <YAxis tick={{ fill: '#475569', fontSize: 9 }} axisLine={false} tickLine={false} domain={[0, 100]} />
                      <Bar dataKey="score" radius={[6, 6, 0, 0]} barSize={40}>
                        {domainAverages.map((entry, index) => (<Cell key={index} fill={entry.color} />))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
               </motion.div>
            ...
            <div className="mt-16 space-y-8">
               <div className="flex flex-col xl:flex-row justify-between items-end gap-6">
                  <div>
                    <h2 className="text-3xl font-black text-white tracking-tighter uppercase flex items-center gap-4">OPERATIVE DOSSIERS <Database className="text-fuchsia-400" size={24} /></h2>
                    <p className="text-slate-500 font-bold text-[10px] tracking-[0.2em] uppercase mt-2">List of all tested operatives in the secure network.</p>
                  </div>
                  <div className="flex gap-4 w-full xl:w-auto">
                     <div className="relative flex-1 xl:w-80">
                        <Search size={14} className="absolute left-5 top-1/2 -translate-y-1/2 text-fuchsia-500" />
                        <input type="text" placeholder="SEARCH IDENTITY..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-black/90 border border-white/10 rounded-full py-4 pl-12 pr-6 text-[10px] font-bold text-white outline-none focus:border-fuchsia-500 transition-all placeholder:text-slate-700" />
                     </div>
                     <select value={activeClass} onChange={(e) => setActiveClass(e.target.value)} className="bg-black/90 border border-white/10 rounded-full px-8 text-[10px] font-bold text-white outline-none cursor-pointer uppercase">
               ...
               <div className="grid grid-cols-1 gap-5 pb-20">
                  {filteredReports.map((r, idx) => {
                    ...
                    return (
                      <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.03 }} className="flex items-center justify-between bg-[#07070B]/70 backdrop-blur-3xl p-6 rounded-[2.5rem] border border-white/5 hover:border-fuchsia-500/30 transition-all group shadow-2xl">
                         <div className="flex items-center gap-6">
                            <div className="w-16 h-16 rounded-full bg-black border border-white/10 flex items-center justify-center relative shadow-inner">
                               <User size={24} className="text-slate-500 group-hover:text-white transition-colors" />
                               <div className={`absolute bottom-0 right-0 w-4 h-4 rounded-full border-2 border-black ${status.bg} shadow-[0_0_10px_currentColor]`} style={{ color: status.color }} />
                            </div>
                            <div>
                               <p className="font-black text-xl text-white tracking-tight uppercase group-hover:text-fuchsia-400 transition-colors">{r.username}</p>
                               <p className="text-[10px] text-slate-500 font-black tracking-widest uppercase mt-1">{r.class_name}</p>
                            </div>
                         </div>

                         <div className="flex items-center gap-8">
                            <div className="hidden lg:grid grid-cols-3 gap-6">
                               <div className="text-center"><p className="text-[8px] font-bold text-slate-500 uppercase mb-1">SOC. ENG</p><p className="text-lg font-black text-fuchsia-400">{r.scores.social ?? '--'}</p></div>
                               <div className="text-center"><p className="text-[8px] font-bold text-slate-500 uppercase mb-1">MALWARE</p><p className="text-lg font-black text-red-500">{r.scores.malware ?? '--'}</p></div>
                               <div className="text-center"><p className="text-[8px] font-bold text-slate-500 uppercase mb-1">PHISHING</p><p className="text-lg font-black text-blue-500">{r.scores.phish ?? '--'}</p></div>
                            </div>
                            <div className="flex items-center justify-center w-20 h-20 rounded-full border-2 border-white/5 relative ml-4">
                               <svg className="w-full h-full absolute -rotate-90">
                                  <circle cx="50%" cy="50%" r="45%" stroke={status.color} strokeWidth="3" fill="transparent" strokeDasharray={`${r.avgScore * 2.8} 1000`} className="opacity-80" />
                               </svg>
                               <span className={`text-2xl font-black ${status.text}`}>{r.avgScore}</span>
                            </div>
                            <button onClick={() => setFeedbackModal(r)} className="px-8 py-4 bg-white/5 border border-white/10 rounded-full hover:bg-white text-slate-400 hover:text-black font-black text-[10px] tracking-widest uppercase transition-all flex items-center gap-3">
                  ...
                  {filteredReports.length === 0 && (
                    <div className="p-32 flex flex-col items-center justify-center text-center border-2 border-dashed border-white/5 rounded-[3rem] bg-black/40">
                       <Info size={48} className="text-slate-600 mb-6" />
                       <h3 className="text-2xl font-black text-white tracking-widest uppercase">DATABASE EMPTY</h3>
                       <p className="text-slate-500 font-bold uppercase text-xs tracking-widest mt-2">NO TESTED OPERATIVES DETECTED IN THE CURRENT CLASS FILTER.</p>
                    </div>
                  )}
               </div>
            </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-black/95 backdrop-blur-3xl">
             <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="w-full max-w-2xl bg-[#0a0a0f] border border-white/10 rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-10">
                   <div><h2 className="text-3xl font-black text-white uppercase tracking-tight">SYSTEM FEEDBACK</h2><p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">TRANSMIT SUGGESTIONS TO CORE DEVELOPERS</p></div>
                   <button onClick={() => setAppFeedbackModal(false)} className="p-2 bg-white/5 rounded-full text-slate-500"><X size={20}/></button>
                </div>
                <div className="space-y-6">
                   <select value={appFeedbackForm.category} onChange={(e) => setAppFeedbackForm({...appFeedbackForm, category: e.target.value})} className="w-full bg-black border border-white/10 rounded-2xl p-4 text-[10px] font-black text-white uppercase outline-none focus:border-emerald-500">
                      <option value="FEATURE SUGGESTION">FEATURE SUGGESTION</option>
                      <option value="BUG REPORT">BUG REPORT / ERROR</option>
                   </select>
                   <textarea value={appFeedbackForm.message} onChange={(e) => setAppFeedbackForm({...appFeedbackForm, message: e.target.value})} placeholder="TRANSMISSION CONTENT..." className="w-full h-40 bg-black border border-white/10 rounded-2xl p-4 text-xs font-bold text-white focus:border-emerald-500 outline-none resize-none" />
                   <button onClick={submitAppFeedback} disabled={isSendingFeedback} className="w-full py-5 bg-emerald-500 text-white rounded-2xl font-black text-xs tracking-[0.3em] uppercase hover:bg-emerald-400 transition-all flex items-center justify-center gap-4">
                      {isSendingFeedback ? "TRANSMITTING..." : "EXECUTE TRANSMISSION"} <Send size={16}/>
                   </button>
                </div>
             </motion.div>
          </motion.div>
