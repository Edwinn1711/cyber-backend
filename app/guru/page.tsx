...
export default function DashboardGuruZenith() {
  ...
  const submitAppFeedback = async () => {
    ...
    try {
      ...
      if (response.ok) { alert(`Feedback Transmitted. Thank you.`); setAppFeedbackModal(false); setAppFeedbackForm({ category: 'AI ENHANCEMENT', message: '' }); } 
    } catch (error) { console.error(error); } 
  ...
  return (
    <div className="flex h-screen w-full bg-[#020108] text-slate-200 overflow-hidden font-sans selection:bg-fuchsia-500/30 text-sm">
            ...
            <motion.div initial={{ scale: 0.95, y: 15 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} className="relative w-full max-w-4xl bg-[#0a0a0f]/95 border border-white/10 rounded-[2rem] p-8 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden backdrop-blur-xl">
               ...
               <div className="flex justify-between items-start mb-6 shrink-0 relative z-10">
                  <div className="flex items-center gap-6">
                     ...
                     <div><p className="text-[10px] font-bold text-fuchsia-400 tracking-widest uppercase mb-1">DOSSIER VIEW</p><h2 className="text-3xl font-black text-white uppercase">{feedbackModal.username}</h2><p className="text-xs font-semibold text-slate-400 mt-1 uppercase">{feedbackModal.class_name}</p></div>
                  </div>
                  <button onClick={() => setFeedbackModal(null)} className="p-2 bg-white/5 rounded-full hover:bg-red-500/20 text-slate-400 transition-all"><X size={20} /></button>
               </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {feedbackModal.history.map((log: any, i: number) => (
                        <div key={i} className="flex items-center justify-between p-4 bg-black/40 border border-white/5 rounded-xl group hover:border-white/20 transition-all">
                          <div className="flex flex-col"><p className="text-xs font-bold text-white">SESSION {i+1}</p><p className="text-[10px] text-slate-500 uppercase mt-1">{log.parsedDomain} | SCORE: <span className={getScoreData(log.parsedScore).text}>{log.parsedScore}</span></p></div>
      ...
      {/* --- MODAL 2: ULTRA LUXURY FEEDBACK (Fuchsia & Violet) --- */}
      <AnimatePresence>
        {appFeedbackModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-[#020108]/95 backdrop-blur-[40px]">
             {/* Glow Dekoratif di Belakang Modal */}
             ...
             <motion.div initial={{ scale: 0.9, y: 30, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.9, y: 20, opacity: 0 }} transition={{ type: 'spring', damping: 20 }} className="w-full max-w-2xl bg-[#0a0a0f]/80 border border-white/10 rounded-[3rem] p-10 shadow-[0_0_60px_rgba(217,70,239,0.15)] relative overflow-hidden backdrop-blur-3xl">
                ...
                <div className="flex justify-between items-start mb-12 relative z-10">
                   <div className="flex items-center gap-6">
                      <div className="w-16 h-16 bg-gradient-to-br from-violet-600 to-fuchsia-600 rounded-[1.5rem] flex items-center justify-center text-white shadow-[0_0_25px_rgba(217,70,239,0.4)]">
                         <Zap size={30} className="fill-current" />
                      </div>
                      <div>
                        <h2 className="text-4xl font-black text-white uppercase tracking-tighter">PREMIUM FEEDBACK</h2>
                        <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em] mt-2">TRANSMITTING SYSTEM OPTIMIZATION DATA</p>
                      </div>
                   </div>
                ...
                <div className="space-y-8 relative z-10">
                   <div>
                      <label className="text-[10px] font-black text-fuchsia-400 uppercase tracking-widest mb-4 flex items-center gap-2"><Hexagon size={12}/> SELECT TRANSMISSION CATEGORY</label>
                      <div className="relative group">
                         <select 
                            value={appFeedbackForm.category} 
                            onChange={(e) => setAppFeedbackForm({...appFeedbackForm, category: e.target.value})} 
                            className="w-full bg-black/60 border border-white/10 rounded-2xl p-5 text-xs font-black text-white uppercase outline-none focus:border-fuchsia-500 focus:shadow-[0_0_20px_rgba(217,70,239,0.2)] transition-all appearance-none cursor-pointer"
                         >
                            ...
                            <option value="REAL-TIME ANALYTICS">📊 ADVANCED ANALYTICS FEATURES</option>
                            ...
                            <option value="BUG REPORT">⚠️ CRITICAL BUG / SYSTEM ERROR</option>
                            <option value="OTHER">🌐 OTHER / MISCELLANEOUS</option>
                         </select>
                         <ChevronRight size={16} className="absolute right-5 top-1/2 -translate-y-1/2 rotate-90 text-slate-600 pointer-events-none group-hover:text-fuchsia-400 transition-colors" />
                      </div>
                   </div>

                   <div>
                      <label className="text-[10px] font-black text-fuchsia-400 uppercase tracking-widest mb-4 flex items-center gap-2"><Terminal size={12}/> TRANSMISSION CONTENT</label>
                      <textarea 
                        value={appFeedbackForm.message} 
                        onChange={(e) => setAppFeedbackForm({...appFeedbackForm, message: e.target.value})} 
                        placeholder="INPUT SYSTEM RECOMMENDATIONS OR REPORT ANOMALIES HERE..." 
                        className="w-full h-44 bg-black/60 border border-white/10 rounded-3xl p-6 text-xs font-bold text-white focus:border-fuchsia-500 focus:shadow-[0_0_20px_rgba(217,70,239,0.2)] outline-none resize-none placeholder:text-slate-800 transition-all" 
                      />
                   </div>
                   ...
                   <button 
                      ...
                      className="w-full py-5 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white rounded-[1.5rem] font-black text-[11px] tracking-[0.5em] uppercase hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-5 shadow-[0_15px_40px_rgba(217,70,239,0.3)] disabled:opacity-50"
                   >
                      {isSendingFeedback ? "TRANSMITTING DATA..." : "EXECUTE TRANSMISSION"} <Send size={18} className={isSendingFeedback ? "animate-bounce" : ""}/>
                   </button>
                </div>

                {/* Ornamen Grafis Mewah di Modal */}
                <div className="absolute -bottom-20 -left-20 w-64 h-64 bg-indigo-600/10 rounded-full blur-[80px] -z-10" />
             </motion.div>
          </motion.div>
        )}
      </AnimatePresence>

      <motion.aside animate={{ width: isSidebarCollapsed ? 80 : 280 }} className="h-screen bg-[#05050A]/80 backdrop-blur-3xl border-r border-white/5 flex flex-col z-[100] shadow-2xl transition-all duration-500">
        <div className="h-20 px-6 flex items-center justify-between border-b border-white/5">
          {!isSidebarCollapsed && <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-600 to-fuchsia-600 flex items-center justify-center shadow-lg"><LayoutDashboard size={20} className="text-white" /></div><span className="font-black text-white uppercase tracking-widest text-[11px]">INSTRUCTOR</span></div>}
          <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="p-2 text-slate-400 mx-auto hover:text-white transition-all">{isSidebarCollapsed ? <ChevronRight size={18} /> : <ChevronLeft size={18} />}</button>
        </div>
        <nav className="flex-1 px-4 py-10 space-y-4">
          <button onClick={() => setView('dashboard')} className={`w-full flex items-center p-4 rounded-2xl transition-all gap-4 ${view === 'dashboard' ? 'bg-fuchsia-600/10 text-fuchsia-400 border border-fuchsia-500/20 shadow-lg' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}><Activity size={22} />{!isSidebarCollapsed && <span className="font-black text-[10px] tracking-widest uppercase">DASHBOARD</span>}</button>
          <button onClick={() => setAppFeedbackModal(true)} className="w-full flex items-center p-4 rounded-2xl text-slate-500 hover:text-fuchsia-400 hover:bg-fuchsia-600/10 transition-all gap-4 group"><Lightbulb size={22} className="group-hover:animate-pulse" />{!isSidebarCollapsed && <span className="font-black text-[10px] tracking-widest uppercase">SYSTEM FEEDBACK</span>}</button>
        </nav>
            ...
            {view === 'dashboard' && (
              <motion.div key="analytics" {...(portalTransition as any)} className="max-w-[1450px] w-full mx-auto space-y-12 pb-24">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 auto-rows-[minmax(200px,auto)]">
                   <motion.div whileHover={{ y: -6 }} className="lg:col-span-4 lg:row-span-2 bg-[#0a0a0f]/60 border border-white/10 rounded-[3rem] p-10 shadow-2xl flex flex-col justify-between backdrop-blur-2xl relative overflow-hidden group">
                      <div className="absolute top-0 right-0 w-40 h-40 bg-violet-600/5 blur-[80px] group-hover:bg-violet-600/10 transition-all" />
                      <p className="text-[11px] font-black text-slate-500 tracking-[0.3em] uppercase mb-6 flex items-center gap-2"><Target size={14}/> PARTICIPATION</p>
                      <div className="space-y-2"><p className="text-7xl font-black text-white tracking-tighter">{participationStats.percentage}%</p><p className="text-[10px] font-black text-fuchsia-400 tracking-[0.2em] uppercase">SYSTEM UPTIME REACHED</p></div>
                      <div className="w-full h-3 bg-black/50 rounded-full overflow-hidden border border-white/5 mt-10"><motion.div initial={{ width: 0 }} animate={{ width: `${participationStats.percentage}%` }} transition={{ duration: 1.8 }} className="h-full bg-gradient-to-r from-violet-600 to-fuchsia-600 shadow-[0_0_15px_#d946ef]" /></div>
                   </motion.div>

                   <motion.div whileHover={{ y: -6 }} className="lg:col-span-4 lg:row-span-2 bg-[#0a0a0f]/60 border border-white/10 rounded-[3rem] p-10 flex flex-col items-center shadow-2xl backdrop-blur-2xl relative overflow-hidden group">
                      <p className="text-[11px] font-black text-slate-500 tracking-[0.3em] uppercase mb-6 w-full text-left flex items-center gap-2"><ShieldAlert size={14}/> FLEET STATUS</p>
                      <div className="relative w-full h-[220px]"><ResponsiveContainer><PieChart><Pie data={readinessDistribution} innerRadius={75} outerRadius={95} dataKey="value" stroke="none" paddingAngle={8}>{readinessDistribution.map((e, i) => (<Cell key={i} fill={e.color} style={{filter:`drop-shadow(0 0 10px ${e.color}50)`}} />))}</Pie></PieChart></ResponsiveContainer><div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-5xl font-black text-white">{participationStats.active}</span><span className="text-[9px] font-black text-slate-500 tracking-widest uppercase">ACTIVE</span></div></div>
                      <div className="flex gap-6 mt-6">{readinessDistribution.map((item, idx) => (<div key={idx} className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: item.color, boxShadow:`0 0 10px ${item.color}` }} /><span className="text-[10px] font-black text-slate-300 uppercase">{item.name}</span></div>))}</div>
                   </motion.div>

                   <motion.div whileHover={{ y: -6 }} className="lg:col-span-4 lg:row-span-2 bg-[#0a0a0f]/60 border border-white/10 rounded-[3rem] p-10 shadow-2xl h-[320px] backdrop-blur-2xl relative overflow-hidden group">
                      <p className="text-[11px] font-black text-slate-500 tracking-[0.3em] uppercase mb-8 flex items-center gap-2"><Cpu size={14}/> DOMAIN MASTERY</p>
                      <ResponsiveContainer><BarChart data={domainAverages} margin={{ left: -35 }}><CartesianGrid strokeDasharray="4 4" vertical={false} stroke="rgba(255,255,255,0.03)" /><XAxis dataKey="domain" tick={{ fontSize: 10, fontBold: '900', fill: '#94a3b8' }} axisLine={false} tickLine={false}/><YAxis tick={{ fontSize: 10, fill: '#475569' }} domain={[0, 100]} axisLine={false} tickLine={false}/><Bar dataKey="score" radius={[8, 8, 0, 0]} barSize={50}>{domainAverages.map((e, i) => (<Cell key={i} fill={`url(#barGradient-${i})`} />))}<defs>{domainAverages.map((e, i) => (<linearGradient id={`barGradient-${i}`} key={i} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={e.color} /><stop offset="100%" stopColor={e.color} stopOpacity={0.1} /></linearGradient>))}</defs></Bar></BarChart></ResponsiveContainer>
                   </motion.div>
                </div>
                ...
                <div className="mt-20 space-y-10 pb-32">
                   <div className="flex flex-col xl:flex-row justify-between items-end gap-10">
                      <div className="space-y-3">
                        <h2 className="text-4xl font-black text-white tracking-tighter uppercase flex items-center gap-5">OPERATIVE DOSSIERS <Database className="text-fuchsia-500 animate-pulse" size={28} /></h2>
                        <p className="text-slate-600 font-black text-[11px] tracking-[0.4em] uppercase">ACCESSING SECURE PERSONNEL RECORD LOGS</p>
                      </div>
                      <div className="flex gap-6 w-full xl:w-auto">
                         <div className="relative flex-1 xl:w-[400px] group"><Search size={18} className="absolute left-6 top-1/2 -translate-y-1/2 text-fuchsia-500 group-focus-within:scale-110 transition-all" /><input type="text" placeholder="INPUT IDENTITY HASH..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-black/80 border border-white/10 rounded-[2rem] py-5 pl-16 pr-8 text-[12px] font-black text-white outline-none focus:border-fuchsia-500 focus:shadow-[0_0_30px_rgba(217,70,239,0.1)] transition-all placeholder:text-slate-800" /></div>
                            ...
                            return (
                               <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.04 }} key={idx} className="flex items-center justify-between bg-[#07070B]/60 backdrop-blur-[50px] p-8 rounded-[3.5rem] border border-white/5 hover:border-fuchsia-500/30 transition-all group shadow-2xl relative overflow-hidden">
                                  ...
                                  <div className="flex items-center gap-10 relative z-10">
                                     <div className="w-20 h-20 rounded-[2rem] bg-black border border-white/10 flex items-center justify-center relative shadow-2xl transform group-hover:rotate-6 transition-all">
                                        <User size={30} className="text-slate-600 group-hover:text-white transition-all" />
                                        <div className={`absolute -bottom-2 -right-2 w-7 h-7 rounded-full border-[3px] border-[#07070B] ${status.bg} shadow-[0_0_15px_currentColor] animate-pulse`} style={{ color: status.color }} />
                                     </div>
                                     <div>
                                        <p className="font-black text-2xl text-white tracking-tighter uppercase group-hover:text-fuchsia-400 transition-all">{r.username}</p>
                                        <p className="text-[11px] text-slate-500 font-black tracking-[0.3em] uppercase mt-2 flex items-center gap-2"><Terminal size={12}/> {r.class_name}</p>
                                     </div>
                                  </div>

                                  <div className="flex items-center gap-14 relative z-10">
                                     <div className="hidden lg:grid grid-cols-3 gap-10">
                                        <div className="text-center group/score"><p className="text-[10px] font-black text-slate-600 uppercase mb-2 tracking-widest group-hover/score:text-fuchsia-400 transition-colors">SOC</p><p className="text-2xl font-black text-fuchsia-400" style={{textShadow:'0 0 15px #d946ef60'}}>{r.scores.social ?? '--'}</p></div>
                                        <div className="text-center group/score"><p className="text-[10px] font-black text-slate-600 uppercase mb-2 tracking-widest group-hover/score:text-red-400 transition-colors">MAL</p><p className="text-2xl font-black text-red-500" style={{textShadow:'0 0 15px #ef444460'}}>{r.scores.malware ?? '--'}</p></div>
                                        <div className="text-center group/score"><p className="text-[10px] font-black text-slate-600 uppercase mb-2 tracking-widest group-hover/score:text-blue-400 transition-colors">PHI</p><p className="text-2xl font-black text-blue-500" style={{textShadow:'0 0 15px #3b82f660'}}>{r.scores.phish ?? '--'}</p></div>
                                     </div>
                                     
                                     <div className="flex items-center justify-center w-24 h-24 rounded-full border-[3px] border-white/5 relative ml-6 transform group-hover:scale-110 transition-all">
                                        <svg className="w-full h-full absolute -rotate-90">
                                           <circle cx="50%" cy="50%" r="44%" stroke={status.color} strokeWidth="5" fill="transparent" strokeDasharray={`${r.avgScore * 2.8} 1000`} className="opacity-80" style={{transition:'stroke-dasharray 1.5s ease-out'}}/>
                                        </svg>
                                        <span className={`text-3xl font-black ${status.text}`} style={{textShadow:`0 0 20px ${status.color}80`}}>{r.avgScore}</span>
                                     </div>
                                     
                                     <button onClick={() => setFeedbackModal(r)} className="px-10 py-5 bg-white/5 border border-white/10 rounded-[1.8rem] hover:bg-white text-slate-300 hover:text-black font-black text-[11px] tracking-[0.3em] uppercase transition-all flex items-center gap-4 group/btn shadow-xl"><Eye size={18} className="group-hover/btn:animate-bounce" /> VIEW DOSSIER</button>
                                  </div>
                               </motion.div>
                            )
                         })}
