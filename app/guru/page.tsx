        ...
        <header className="h-16 flex items-center justify-between px-6 lg:px-8 border-b border-white/5 bg-black/40 backdrop-blur-xl">
            ...
            <button onClick={() => setProfileModal(true)} className="flex items-center gap-4 text-left group hover:opacity-80 transition-all">
                <div className="text-right hidden sm:block">
                  <p className="text-[10px] font-black text-white tracking-widest uppercase group-hover:text-fuchsia-400 transition-colors">{user.username}</p>
                  <p className="text-[7px] font-bold text-fuchsia-400 uppercase tracking-[0.3em] mt-0.5 flex items-center justify-end gap-1.5"><Radio size={8} className="animate-pulse"/> INSTRUCTOR NODE</p>
                </div>
                <div className="w-8 h-8 bg-gradient-to-br from-violet-600 to-fuchsia-600 rounded-full flex items-center justify-center border border-white/20 shadow-[0_0_15px_rgba(217,70,239,0.3)] group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(217,70,239,0.6)] transition-all">
                  <User size={16} />
                </div>
            </button>
        </header>

        <main className="flex-1 overflow-y-auto no-scrollbar px-6 lg:px-8 py-6" ref={scrollRef}>
          <AnimatePresence mode="wait">
            {view === 'dashboard' && (
              <motion.div key="analytics" {...(portalTransition as any)} className="max-w-[1200px] w-full mx-auto space-y-6 pb-20">
                
                {/* HEADER */}
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
                  <div className="space-y-1.5"><div className="text-fuchsia-400 font-black text-[9px] tracking-[0.4em] uppercase flex items-center gap-2"><Globe size={12} className="animate-spin-slow" /> PLANETARY MONITORING ACTIVE</div><h1 className="text-2xl lg:text-3xl font-black text-white tracking-tighter uppercase leading-tight">{getGreeting()}, <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 via-fuchsia-500 to-indigo-400">{user.username}</span>.</h1><p className="text-slate-500 font-medium text-[10px] tracking-wide max-w-xl">Accessing encrypted dossiers and fleet readiness metrics from the secure network.</p></div>
                  <button onClick={fetchData} className="px-5 py-2.5 bg-white/5 border border-white/10 text-white rounded-full font-black text-[9px] tracking-[0.3em] hover:bg-white hover:text-black transition-all flex items-center gap-2 uppercase"><RefreshCcw size={12} className={loading ? "animate-spin text-fuchsia-400" : ""}/> REFRESH DATASET</button>
                </div>

                {/* BENTO STATS */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 auto-rows-[minmax(140px,auto)]">
                   <motion.div whileHover={{ y: -2 }} className="lg:col-span-4 lg:row-span-2 bg-[#0a0a0f]/80 border border-white/10 rounded-3xl p-6 shadow-xl flex flex-col justify-between backdrop-blur-3xl relative overflow-hidden group">
                      <div className="absolute top-0 right-0 w-24 h-24 bg-fuchsia-600/10 blur-[80px]" />
                      <p className="text-[9px] font-black text-slate-500 tracking-[0.3em] uppercase mb-4 flex items-center gap-2"><Target size={12} className="text-fuchsia-400"/> PARTICIPATION</p>
                      <div className="space-y-1"><p className="text-4xl font-black text-white tracking-tighter">{participationStats.percentage}%</p><p className="text-[8px] font-black text-fuchsia-400 tracking-[0.2em] uppercase">SYSTEM UPTIME REACHED</p></div>
                      <div className="w-full h-2 bg-black/60 rounded-full overflow-hidden border border-white/10 mt-6"><motion.div initial={{ width: 0 }} animate={{ width: `${participationStats.percentage}%` }} transition={{ duration: 1.8 }} className="h-full bg-gradient-to-r from-violet-600 to-fuchsia-600 shadow-[0_0_15px_#d946ef]" /></div>
                   </motion.div>

                   <motion.div whileHover={{ y: -2 }} className="lg:col-span-4 lg:row-span-2 bg-[#0a0a0f]/80 border border-white/10 rounded-3xl p-6 flex flex-col items-center shadow-xl backdrop-blur-2xl relative overflow-hidden group">
                      <p className="text-[9px] font-black text-slate-500 tracking-[0.3em] uppercase mb-4 w-full text-left flex items-center gap-2"><ShieldAlert size={12} className="text-indigo-400"/> FLEET STATUS</p>
                      <div className="relative w-full h-[140px]"><ResponsiveContainer><PieChart><Pie data={readinessDistribution} innerRadius={50} outerRadius={65} dataKey="value" stroke="none" paddingAngle={8}>{readinessDistribution.map((e, i) => (<Cell key={i} fill={e.color} style={{filter:`drop-shadow(0 0 10px ${e.color}50)`}} />))}</Pie></PieChart></ResponsiveContainer><div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-3xl font-black text-white">{participationStats.active}</span><span className="text-[7px] font-black text-slate-500 tracking-widest uppercase">ACTIVE</span></div></div>
                      <div className="flex gap-4 mt-4">{readinessDistribution.map((item, idx) => (<div key={idx} className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: item.color, boxShadow:`0 0 8px ${item.color}` }} /><span className="text-[8px] font-black text-slate-300 uppercase">{item.name}</span></div>))}</div>
                   </motion.div>

                   <motion.div whileHover={{ y: -2 }} className="lg:col-span-4 lg:row-span-2 bg-[#0a0a0f]/80 border border-white/10 rounded-3xl p-6 shadow-xl h-[220px] backdrop-blur-2xl relative overflow-hidden group">
                      <p className="text-[9px] font-black text-slate-500 tracking-[0.3em] uppercase mb-4 flex items-center gap-2"><Cpu size={12} className="text-blue-400"/> DOMAIN MASTERY</p>
                      <ResponsiveContainer><BarChart data={domainAverages} margin={{ left: -35 }}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(255,255,255,0.03)" /><XAxis dataKey="domain" tick={{ fontSize: 8, fontWeight: '900', fill: '#94a3b8' }} axisLine={false} tickLine={false}/><YAxis tick={{ fontSize: 8, fill: '#475569' }} domain={[0, 100]} axisLine={false} tickLine={false}/><Bar dataKey="score" radius={[6, 6, 0, 0]} barSize={30}>{domainAverages.map((e, i) => (<Cell key={i} fill={`url(#barGradient-${i})`} />))}<defs>{domainAverages.map((e, i) => (<linearGradient id={`barGradient-${i}`} key={i} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={e.color} /><stop offset="100%" stopColor={e.color} stopOpacity={0.1} /></linearGradient>))}</defs></Bar></BarChart></ResponsiveContainer>
                   </motion.div>
                </div>
                ...
                <div className="mt-12 space-y-6 pb-20">
                   <div className="flex flex-col xl:flex-row justify-between items-end gap-6">
                      <div className="space-y-1.5"><h2 className="text-2xl font-black text-white tracking-tighter uppercase flex items-center gap-3">OPERATIVE DOSSIERS <Database className="text-fuchsia-500" size={20} /></h2><p className="text-slate-600 font-black text-[9px] tracking-[0.3em] uppercase">ACCESSING SECURE PERSONNEL RECORD LOGS</p></div>
                      <div className="flex gap-3 w-full xl:w-auto">
                         <div className="relative flex-1 xl:w-[300px] group"><Search size={14} className="absolute left-5 top-1/2 -translate-y-1/2 text-fuchsia-500 transition-all" /><input type="text" placeholder="IDENTITY HASH..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-black/80 border border-white/10 rounded-full py-3 pl-12 pr-6 text-[10px] font-black text-white outline-none focus:border-fuchsia-500 transition-all placeholder:text-slate-800" /></div>
                         <select value={activeClass} onChange={(e) => setActiveClass(e.target.value)} className="bg-black/80 border border-white/10 rounded-full px-6 text-[9px] font-black text-white outline-none cursor-pointer uppercase hover:border-fuchsia-500 transition-all">{dynamicClasses.map(c => <option key={c} value={c} className="bg-black">{c}</option>)}</select>
                      </div>
                   </div>
                   ...
                   <div className="flex flex-col gap-4">
                      <AnimatePresence>
                         {filteredReports.map((r, idx) => {
                            ...
                            return (
                               <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.04 }} key={idx} className="flex items-center justify-between bg-[#07070B]/60 backdrop-blur-[50px] p-4 lg:p-5 rounded-3xl border border-white/5 hover:border-fuchsia-500/20 transition-all group shadow-2xl relative overflow-hidden">
                                  <div className="absolute left-0 top-0 w-1.5 h-full opacity-30 shadow-[0_0_20px_currentColor]" style={{ backgroundColor: status.color, color: status.color }} />
                                  
                                  <div className="flex items-center gap-5 relative z-10">
                                     <div className="w-12 h-12 rounded-2xl bg-black border border-white/10 flex items-center justify-center relative shadow-xl group-hover:scale-105 transition-all">
                                        <User size={20} className="text-slate-600 group-hover:text-white transition-all" />
                                     ...
                                     <div>
                                        <p className="font-black text-lg text-white tracking-tighter uppercase group-hover:text-fuchsia-400 transition-all">{r.username}</p>
                                        <p className="text-[9px] text-slate-500 font-black tracking-[0.3em] uppercase mt-1 flex items-center gap-1.5"><Terminal size={10}/> {r.class_name}</p>
                                     </div>
                                  </div>

                                  <div className="flex items-center gap-8 relative z-10">
                                     <div className="hidden lg:grid grid-cols-3 gap-6 text-center">
                                        <div className="group/score"><p className="text-[8px] font-black text-slate-600 uppercase mb-0.5">SOC</p><p className="text-lg font-black text-fuchsia-400">{r.scores.social ?? '--'}</p></div>
                                        <div className="group/score"><p className="text-[8px] font-black text-slate-600 uppercase mb-0.5">MAL</p><p className="text-lg font-black text-red-500">{r.scores.malware ?? '--'}</p></div>
                                        <div className="group/score"><p className="text-[8px] font-black text-slate-600 uppercase mb-0.5">PHI</p><p className="text-lg font-black text-blue-500">{r.scores.phish ?? '--'}</p></div>
                                     </div>
                                     
                                     <div className="flex items-center justify-center w-12 h-12 rounded-full border-[2.5px] border-white/5 relative ml-2 transform group-hover:scale-110 transition-all">
                                        <svg className="w-full h-full absolute -rotate-90">
                                           <circle cx="50%" cy="50%" r="42%" stroke={status.color} strokeWidth="3" fill="transparent" strokeDasharray={`${r.avgScore * 2.6} 1000`} className="opacity-80" />
                                        </svg>
                                        <span className={`text-sm font-black ${status.text}`}>{r.avgScore}</span>
                                     </div>
                                     
                                     <button onClick={() => { setFeedbackModal(r); setExpandedLog(null); }} className="px-6 py-2.5 bg-white/5 border border-white/10 rounded-xl hover:bg-white text-slate-300 hover:text-black font-black text-[9px] tracking-[0.3em] uppercase transition-all flex items-center gap-3 group/btn shadow-md"><Eye size={14} className="group-hover/btn:animate-pulse" /> VIEW DOSSIER</button>
                                  </div>
                               </motion.div>
                            )
                         })}
                      ...
                      {filteredReports.length === 0 && (
                         <div className="p-20 text-center border-2 border-dashed border-white/5 rounded-3xl bg-black/50 backdrop-blur-3xl"><Info size={40} className="mx-auto text-slate-800 mb-6 animate-pulse" /><h3 className="text-xl font-black text-white uppercase tracking-tighter">DATABASE STATUS: VOID</h3><p className="text-slate-700 font-black uppercase text-[10px] tracking-[0.3em] mt-2">NO COMPATIBLE OPERATIVE DATA DETECTED IN THIS SECTOR.</p></div>
                      )}
                   </div>
                </div>
              </motion.div>
            ...
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} className="relative w-full max-w-2xl bg-[#0a0a0f]/95 border border-white/10 rounded-[3rem] p-10 shadow-2xl flex flex-col overflow-hidden">
               ...
               <div className="grid grid-cols-2 gap-4 relative z-10">
                  <div className="bg-black/50 border border-white/5 p-5 rounded-2xl">
                    <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">SUPERVISED FLEET</p>
                    <p className="text-2xl font-black text-white">{reports.length} <span className="text-xs text-slate-500 tracking-widest">CADETS</span></p>
                  </div>
                  <div className="bg-black/50 border border-white/5 p-5 rounded-2xl">
                    <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">PROCESSED AUDITS</p>
                    <p className="text-2xl font-black text-fuchsia-400">{totalAudits} <span className="text-xs text-slate-500 tracking-widest">LOGS</span></p>
                  </div>
               </div>

               {/* Assigned Classes */}
               <div className="mt-6 relative z-10">
                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] mb-3 flex items-center gap-2"><Globe size={12}/> ASSIGNED SECTORS</p>
                 <div className="flex flex-wrap gap-2">
                   {AVAILABLE_CLASSES.map(cls => (
                     <span key={cls} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-[9px] font-black text-slate-300 tracking-widest uppercase">{cls}</span>
                   ))}
                 </div>
               </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/95 backdrop-blur-[60px]">
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} className="relative w-full max-w-3xl bg-[#0a0a0f]/95 border border-white/10 rounded-3xl p-8 shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
               ...
               <div className="flex justify-between items-start mb-8 shrink-0 relative z-10">
                  <div className="flex items-center gap-6">
                     <div className="w-12 h-12 bg-gradient-to-br from-fuchsia-600 to-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-xl border border-white/10"><User size={24} /></div>
                     <div><p className="text-[9px] font-black text-fuchsia-400 tracking-[0.4em] uppercase mb-1">PERSONNEL DOSSIER ACCESS</p><h2 className="text-2xl font-black text-white tracking-tighter uppercase">{feedbackModal.username}</h2><p className="text-[9px] font-black text-slate-500 uppercase mt-1 tracking-[0.2em]">{feedbackModal.class_name} • {feedbackModal.testCount} SESSIONS</p></div>
                  </div>
                  <button onClick={() => { setFeedbackModal(null); setExpandedLog(null); }} className="p-2 bg-white/5 rounded-full text-slate-500 hover:bg-white/10 transition-all border border-white/5"><X size={16} /></button>
               </div>
               ...
               <div className="flex-1 overflow-y-auto pr-3 custom-scrollbar relative z-10 space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {Object.entries(feedbackModal.scores).map(([k, v]: any, idx) => {
                       ...
                       return (
                          <div key={idx} className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col items-center shadow-inner relative overflow-hidden">
                             <div className={`absolute bottom-0 left-0 h-1 transition-all duration-1000 ${st.bg}`} style={{ width: `${v || 0}%` }} />
                             <p className="text-[9px] text-slate-500 font-black uppercase mb-2 tracking-[0.2em]">{k}</p>
                             <p className={`text-3xl font-black ${st.text}`} style={{textShadow:`0 0 15px ${st.color}50`}}>{v !== null ? v : '--'}</p>
                          </div>
                       )
                    })}
                  </div>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-[10px] font-black text-slate-400 flex items-center gap-3 uppercase tracking-[0.3em]"><Database size={14} className="text-indigo-400"/> HISTORICAL AUDIT LOGS</h3>
                    <div className="grid grid-cols-1 gap-4">
                      {feedbackModal.history.map((log: any, i: number) => {
                          ...
                          return (
                            <div key={i} className={`flex flex-col p-4 bg-black/40 border transition-all rounded-2xl ${isExpanded ? 'border-white/20 shadow-lg bg-black/60' : 'border-white/5 hover:border-white/10'}`}>
                              {/* Header Session List */}
                              <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpandedLog(isExpanded ? null : i)}>
                                <div className="flex items-center gap-5">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-mono text-[10px] font-black border transition-all ${isExpanded ? 'bg-fuchsia-500/10 text-fuchsia-400 border-fuchsia-500/30' : 'bg-white/5 text-slate-500 border-white/10'}`}>0{i+1}</div>
                                    <div className="flex flex-col">
                                        <p className="text-[11px] font-black text-white uppercase tracking-wider">{log.parsedDomain}</p>
                                        <p className="text-[9px] text-slate-500 uppercase mt-1 tracking-widest flex items-center gap-2">SCORE: <span className={`font-black ${st.text}`}>{log.parsedScore}</span></p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                                    {totalQs > 0 && (
                                        <div className="hidden sm:flex items-center gap-3 px-3 py-1.5 bg-black/60 rounded-full border border-white/5 text-[9px] font-black tracking-widest uppercase">
                                            <span className="text-emerald-400">{correctQs} BENAR</span>
                                            <span className="text-red-400">{wrongQs} SALAH</span>
                                        </div>
                                    )}
                                    <button onClick={(e) => { e.stopPropagation(); handleDeleteLog(log.dbId); }} className="p-2.5 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-600 hover:text-white transition-all"><Trash2 size={14} /></button>
                                    <div className={`text-slate-500 transition-transform ${isExpanded ? 'rotate-180' : ''}`}><ChevronDown size={16} /></div>
                                </div>
                              </div>
                                ...
                                {isExpanded && (
                                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                        <div className="pt-5 mt-5 border-t border-white/10 space-y-3">
                                            {totalQs === 0 ? (
                                                <p className="text-[10px] text-center text-slate-500 uppercase tracking-widest py-4">NO DETAILED DATA RECORDED FOR THIS SESSION.</p>
                                            ) : (
                                                detailsArray.map((d: any, j: number) => (
                                                    <div key={j} className={`p-4 rounded-xl border flex gap-4 ${d.is_correct ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
                                                        <div className="mt-0.5">
                                                            {d.is_correct ? <CheckCircle2 size={16} className="text-emerald-500"/> : <XCircle size={16} className="text-red-500"/>}
                                                        </div>
                                                        <div className="flex-1 space-y-2">
                                                            <p className="text-[11px] font-medium text-slate-200 leading-relaxed">"{d.question}"</p>
                                                            <div className="bg-black/50 p-2.5 rounded-lg border border-white/5 inline-block">
                                                                <p className="text-[8px] text-slate-500 font-black uppercase tracking-widest mb-1">PERSONNEL RESPONSE:</p>
                                                                <p className={`text-[10px] font-black uppercase tracking-wider ${d.is_correct ? 'text-emerald-400' : 'text-red-400'}`}>{d.answer || "N/A"}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ))
